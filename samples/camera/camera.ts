...
const drones = Array.from({ length: 9 }, (_, i) => ({
  x: 500 + i * 360,
  y: 180 + ((i * 157) % 600),
  phase: i * 1.7,
}));
...
function hitCourier(d: { x: number; y: number; phase: number }) {
  lives--;
  courier.invuln = 2;
  Sfx.hit();
  UI.floatText("DAMAGE", courier.x, courier.y - 24, { color: "#ff6b6b" });
  d.x = courier.x + 180;
  d.y = courier.y - 140;
  courier.x = Math.max(40, courier.x - 100);
  fx.burst({
    at: courier,
    count: 24,
    color: ["#ff6b6b", "#ffe066"],
...
function eachColumn(
  factor: number,
  spacing: number,
  cb: (x: number, seed: number, base: number) => void,
  pad = 2,
) {
  ...
  update() {
    ...
    if (state === "play") {
      ...
      if (Pointer.pressed) {
        ...
        if (
          Math.hypot(wp.x - beacon.x, wp.y - beacon.y) < 80 &&
          Math.hypot(courier.x - beacon.x, courier.y - beacon.y) < 115
        ) {
          fx.burst({
            at: beacon,
            count: 50,
            color: ["#ffe066", "#4ecdc4", "#b197fc"],
            speed: [1.3, 5],
            life: [500, 1000],
            gravity: 0.03,
          });
          Sfx.pickup();
          UI.floatText("DOCKED", beacon.x, beacon.y - 38, { color: "#64f0c8" });
          beaconIndex++;
    ...
    if (Keys.pressed("Equal") || Keys.pressed("NumpadAdd"))
      Camera.zoom = Mathf.clamp(Camera.zoom + 0.1, 0.65, 1.7);
    if (Keys.pressed("Minus") || Keys.pressed("NumpadSubtract"))
      Camera.zoom = Mathf.clamp(Camera.zoom - 0.1, 0.65, 1.7);
  },
  draw() {
    const { ctx } = Draw;
    ...
    Camera.layer(0.12, () => {
      eachColumn(0.12, 130, (x, seed, base) => {
        ...
        ctx.fillStyle = "#121b3b";
        ctx.beginPath();
        ctx.moveTo(x, base);
        ctx.lineTo(x + 70, base - h);
    ...
    Camera.render(() => {
      ...
      for (let i = 0; i < beacons.length; i++) {
        const b = beacons[i],
          ...
          active = i === beaconIndex;
        ctx.strokeStyle = done ? "#64f0c8" : active ? "#ffe066" : "#405477";
        ctx.lineWidth = active ? 3 : 2;
        ctx.beginPath();
        ctx.arc(b.x, b.y, active ? 28 + Math.sin(performance.now() / 180) * 6 : 20, 0, Math.PI * 2);
        ctx.stroke();
        Draw.circle(b, 7, done ? "#64f0c8" : active ? "#ffe066" : "#405477");
        // World-space text = Draw.text (UI.text is ALWAYS screen space).
        Draw.text(done ? "DELIVERED" : active ? "DOCK HERE" : "LOCKED", {
          x: b.x - 38,
          y: b.y - 55,
          size: 13,
          color: "#fff",
      ...
      for (const d of drones) {
        ...
        ctx.rotate(Math.atan2(courier.y - d.y, courier.x - d.x));
      ...
      if (courier.invuln <= 0 || Math.floor(elapsed * 12) % 2) {
        ctx.save();
    ...
    if (state !== "play") {
      ...
      UI.text(state === "won" ? "DELIVERY COMPLETE" : "COURIER LOST", {
        x: view.w / 2,
        y: view.h / 2 - 40,
        size: 30,
        bold: true,
        color: state === "won" ? "#64f0c8" : "#ff6b6b",
        align: "center",
      });
      UI.text("Press R to run the route again", {
        x: view.w / 2,
        y: view.h / 2 + 10,
        size: 14,
        color: "#fff",
