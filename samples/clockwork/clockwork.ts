...
interface Bud {
  x: number;
  y: number;
  grow: ReturnType<typeof Anim.animate>;
  hue: number;
  life: number;
}
...
function resetRun() {
  ...
  hud.health = 5;
  hud.elapsed = 0;
  hud.message = "THE GARDEN AWAKENS";
  hud.messageAlpha = 1;
  state = "play";
  spawnBud();
}
...
Loop.run({
  ...
  draw(ctx) {
    ...
    if (hud.messageAlpha > 0) {
      ctx.globalAlpha = hud.messageAlpha;
      UI.text(hud.message, { x: 14, y: vp.h - 40, size: 20, bold: true, color: "#ffe066" });
      ctx.globalAlpha = 1;
    }
    if (hud.pulse > 0) {
      ctx.strokeStyle = `rgba(255,224,102,${hud.pulse})`;
      ctx.lineWidth = 3;
      ctx.strokeRect(5, 5, vp.w - 10, vp.h - 10);
    }
    // Hit flash: a red border that blinks and fades when the garden loses a bud.
    if (damage.active) {
      ctx.strokeStyle = `rgba(255,80,80,${damage.value})`;
      ctx.lineWidth = 10;
      ctx.strokeRect(5, 5, vp.w - 10, vp.h - 10);
    }
    if (state === "gameover") {
      ctx.fillStyle = "rgba(7,10,24,.82)";
      ctx.fillRect(0, 0, vp.w, vp.h);
      UI.text("GARDEN SILENT", {
        x: vp.w / 2,
        y: vp.h / 2 - 52,
        size: 34,
        bold: true,
      ...
      UI.text("SPACE or click to grow another run", {
        x: vp.w / 2,
        y: vp.h / 2 + 29,
        size: 13,
        color: "dim",
