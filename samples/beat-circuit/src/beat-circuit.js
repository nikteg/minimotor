...
const { Goodies, Collision, Input, Loop, UI, Mathf } = Minimotor;
...
Loop.run({ update(stepMs) {
  ...
  if (actions.down("gas")) car.speed += 150 * dt; else car.speed = Mathf.damp(car.speed, 0, 0.9, dt);
  ...
  for (let i = 0; i < gs.length; i++) if (Collision.circleHit(car.x, car.y, 0, gs[i].x, gs[i].y, 30)) touching = i;
  if (touching !== gateLock && touching === route.next && route.visit(touching)) {
    const grade = Goodies.timingGrade(Goodies.beatClock(elapsed, BEAT).offset); lastGrade = grade.toUpperCase();
  ...
  const pulse = Goodies.beatClock(elapsed, BEAT).pulse; ctx.strokeStyle = `rgba(255,224,102,${0.2+pulse*0.7})`; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(cx,cy,30+pulse*45,0,Math.PI*2); ctx.stroke();
