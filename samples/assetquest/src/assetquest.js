...
Loop.run({
  update() {
    ...
    if (score === relics.length && Collision.circleHit(player.x, player.y, BODY_R, gate.x, gate.y, 13)) {
      state = "won"; Sfx.win();
      fx.burst({ at: gate, count: 45, color: ["#64f0c8", "#ffe066", "#fff"], speed: [0.7, 3.7], life: [500, 1200], gravity: 0.017 });
    }
  },
  draw() {
    if (!ready) {
      Draw.text("LOADING MOONLIT ARCHIVE", { x: vp.w / 2, y: vp.h / 2 - 24, font: "bold 24px monospace", color: "#ffe066", align: "center" });
      UI.panel({ x: vp.w / 2 - 170, y: vp.h / 2 + 4, w: 340, h: 34, bg: "#172640", border: "#38557e" });
    ...
    Draw.text(`MOON KEYS ${score}/${relics.length}   TIME ${elapsed.toFixed(1)}s`, { x: 14, y: 14, font: "bold 16px monospace", color: "#fff" });
    Draw.text("WASD / ARROWS: MOVE · R: RESTART", { x: 14, y: 36, font: "12px monospace", color: "#9fb3d9" });
    Draw.text(gateOpen ? "All keys found — reach the glowing archive gate" : level.message, { x: 14, y: vp.h - 26, font: "13px monospace", color: gateOpen ? "#64f0c8" : "#9fb3d9" });
    if (state === "won") {
      Draw.rect(0, 0, vp.w, vp.h, "rgba(8,14,27,.82)");
      Draw.text("ARCHIVE RESTORED", { x: vp.w / 2, y: vp.h / 2 - 24, font: "bold 30px monospace", color: "#64f0c8", align: "center" });
      Draw.text(`Run time ${elapsed.toFixed(1)}s`, { x: vp.w / 2, y: vp.h / 2 + 10, font: "16px monospace", color: "#fff", align: "center" });
      Draw.text("Press R to explore again", { x: vp.w / 2, y: vp.h / 2 + 42, font: "13px monospace", color: "#9fb3d9", align: "center" });
    }
  },
});
