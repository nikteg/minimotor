...
function nextWave() {
  ...
  enemies = Goodies.ringFormation(scale.count, cx, cy, Math.max(vp.w, vp.h) * 0.56, wave * 0.37).map((p) => ({ x: p.x, y: p.y, vx: 0, vy: 0, hp: Math.ceil(scale.health), speed: 0.4 * scale.speed }));
}
nextWave();
Loop.run({ update() {
  const cx = vp.w / 2, cy = vp.h / 2;
  cooldown -= 1;
  for (const e of enemies) { const a = Math.atan2(cy - e.y, cx - e.x); e.vx = Math.cos(a) * e.speed; e.vy = Math.sin(a) * e.speed; e.x += e.vx; e.y += e.vy; }
  enemies = enemies.filter((e) => { if (Collision.circleHit(e.x, e.y, 0, cx, cy, 25)) { lives--; fx.burst({ at: { x: cx, y: cy }, count: 12, color: ["#ff6b6b", "#fff"] }); return false; } return true; });
  const target = Goodies.nearest(cx, cy, enemies, (e) => e);
  if (target) {
    const lead = Goodies.leadTarget(cx, cy, target.x, target.y, target.vx, target.vy, BULLET_SPEED);
    if (lead) {
      const wanted = Math.atan2(lead.y - cy, lead.x - cx);
      turret = Goodies.approachAngle(turret, wanted, TURRET_TURN);
      if (cooldown <= 0 && Math.abs(Goodies.wrappedDelta(turret, wanted, Math.PI * 2)) < 0.08) { bullets.push({ x: cx, y: cy, vx: Math.cos(turret) * BULLET_SPEED, vy: Math.sin(turret) * BULLET_SPEED, ttl: 120 }); cooldown = FIRE_COOLDOWN; }
    }
  }
  for (const b of bullets) { b.x += b.vx; b.y += b.vy; b.ttl -= 1; }
  for (let bi = bullets.length - 1; bi >= 0; bi--) for (let ei = enemies.length - 1; ei >= 0; ei--) {
    if (Collision.circleHit(bullets[bi].x, bullets[bi].y, 0, enemies[ei].x, enemies[ei].y, 12)) {
      ...
      fx.burst({ at: bullets[bi], count: hit.critical ? 12 : 5, color: hit.critical ? ["#ffe066", "#fff"] : "#4ecdc4", life: [120, 300] });
  ...
  if (lives <= 0 || Keys.pressed("KeyR")) { wave = 0; lives = 10; score = 0; bullets = []; nextWave(); }
}, draw(ctx) {
  const cx = vp.w / 2, cy = vp.h / 2;
