 ...
 * bytes came from. */
export function parseGlb(bytes: ArrayBuffer, label = "GLB"): GlbContainer {
  ...
  if (declared < 12) throw new Error(`${label}: GLB declares an impossible length ${declared}.`);
  ...
  while (offset < declared) {
    ...
    first = false;
    if (type === CHUNK_JSON) {
      if (json !== undefined) throw new Error(`${label}: GLB has more than one JSON chunk.`);
      json = new TextDecoder().decode(new Uint8Array(bytes, start, length));
    } else if (type === CHUNK_BIN) {
      if (binary !== undefined) throw new Error(`${label}: GLB has more than one BIN chunk.`);
  ...
  if (json === undefined) throw new Error(`${label}: GLB has no JSON chunk.`);
