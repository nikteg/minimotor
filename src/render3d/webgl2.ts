...
export function createWebGL2Renderer(opts: WebGL2RendererOptions = {}): Renderer3D {
  ...
  let blurKit: {
    ...
    vao: WebGLVertexArrayObject;
    /** `sharp` is the full-size copy of the canvas; `levels[i]` is level i+1;
     *  `spare` is the Gaussian's ping-pong partner at the deepest level, and
     *  `full` its result when that level is 0 (a small radius). */
    sharp: BlurSurface | null;
    levels: BlurSurface[];
    spare: BlurSurface | null;
    full: BlurSurface | null;
  } | null = null;

  function blurSurface(w: number, h: number, previous: BlurSurface | null): BlurSurface {
  ...
  function runBlur(
    plan: NonNullable<ReturnType<typeof planBlur>>,
    fullW: number,
    fullH: number,
    fullY: number,
    cssW: number,
    ...
    const draw = (into: BlurSurface | null, w: number, h: number, y = 0): void => {
    ...
    let blurred: BlurSurface;
    if (plan.level === 0) kit.full = blurSurface(fullW, fullH, kit.full);
    ...
    const at = (name: string) => gl!.getUniformLocation(kit.composite, name);
...
/** One colour texture and the framebuffer that writes it — a level of `blur`. */
