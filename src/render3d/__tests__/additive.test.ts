...
describe("additive blending", () => {
  ...
  it("never ghosts additively", () => {
    ...
    // `lastMaterial = null` sits between the two — every pass clears the
    // material run, so uniforms and texture bindings set for one pass are never
    // assumed by the next — so the assertion allows that line.
    expect(read("webgl2.ts")).toMatch(
