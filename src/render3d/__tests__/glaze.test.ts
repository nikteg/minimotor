 ...
 * declaration differently and neither is parsed — the body between the first
 * `{` after the name and the matching close is all this needs. */
...
describe("Material.settle", () => {
  ...
  it("keys the wash on the normal's Y and on world height, in both backends", () => {
    for (const name of ["webgl2.ts", "webgpu.ts"] as const) {
      ...
      // floor it stands on.
      expect(source, name).toContain("settle2.y");
