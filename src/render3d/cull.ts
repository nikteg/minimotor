...
const cache = new WeakMap<MeshData, { version: number; bounds: Bounds3D | null }>();
...
export function meshBounds(mesh: MeshData): Bounds3D | null {
...
export function frustumPlanes(
  ...
  const set = (index: number, a: number, b: number, c: number, d: number): void => {
...
export function inFrustum(
  ...
  world: Mat4 | Float32Array | undefined,
  ...
  const ex =
    Math.abs(m[0]!) * bounds.ex + Math.abs(m[4]!) * bounds.ey + Math.abs(m[8]!) * bounds.ez;
  const ey =
    Math.abs(m[1]!) * bounds.ex + Math.abs(m[5]!) * bounds.ey + Math.abs(m[9]!) * bounds.ez;
  const ez =
    Math.abs(m[2]!) * bounds.ex + Math.abs(m[6]!) * bounds.ey + Math.abs(m[10]!) * bounds.ez;
  ...
  for (let plane = 0; plane < 6; plane++) {
    ...
    const reach = Math.abs(a) * (ex + grow) + Math.abs(b) * (ey + grow) + Math.abs(c) * (ez + grow);
