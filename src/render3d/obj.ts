...
export function parseObj(source: string, options: ObjOptions = {}): MeshData {
  ...
  const resolveIndex = (value: string, count: number, kind: string, line: number): number => {
  ...
  const vertexFor = (token: string, line: number): number => {
  ...
  source.split(/\r?\n/).forEach((rawLine, lineIndex) => {
    ...
    switch (command) {
      case "v": {
        if (parts.length < 3) throw new Error(`OBJ line ${lineNumber}: vertex needs x y z`);
        sourcePositions.push([Number(parts[0]), Number(parts[1]), Number(parts[2])]);
        break;
      }
      case "vt": {
        if (parts.length < 1) throw new Error(`OBJ line ${lineNumber}: UV needs u`);
        sourceUvs.push([Number(parts[0]), Number(parts[1] ?? 0)]);
        break;
      }
      case "vn": {
        if (parts.length < 3) throw new Error(`OBJ line ${lineNumber}: normal needs x y z`);
  ...
  if (indices.length === 0) throw new Error("OBJ contains no faces");
  const mesh: MeshData = {
    positions: new Float32Array(positions),
    indices: positions.length / 3 > 65535 ? new Uint32Array(indices) : new Uint16Array(indices),
  };
  if (hasUvs) mesh.uvs = new Float32Array(uvs);
  if (hasNormals && !missingNormals) mesh.normals = new Float32Array(normals);
  else computeNormals(mesh);
  return mesh;
}
