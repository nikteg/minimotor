...
export function createClip(name: string, tracks: Track[], duration?: number): Clip {
  const derived =
    duration ??
...
export function sampleClip(scene: Scene3D, clip: Clip, time: number, loop = true): void {
  let t = time;
  if (clip.duration > 0) {
    // A negative time (rewinding, or a clock that started mid-frame) must wrap
    // forward, which `%` alone does not do in JS.
    if (loop) t = ((time % clip.duration) + clip.duration) % clip.duration;
    else t = Math.min(clip.duration, Math.max(0, time));
  }
...
function applyTrack(node: Scene3D["nodes"][number], track: Track, t: number): void {
  ...
  if (i1 <= 0) return writeKey(node, track, v, 0, stride);
  if (i1 >= n) return writeKey(node, track, v, n - 1, stride);
  const i0 = i1 - 1;

  if (track.interpolation === "step") return writeKey(node, track, v, i0, stride);
