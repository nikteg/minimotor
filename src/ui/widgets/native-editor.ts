 ...
 * this fires. */
function guardEditorFocus(): void {
  if (focusGuarded || typeof document === "undefined") return;
  focusGuarded = true;
  ...
  let rescuing = false;
  const isEditor = (node: EventTarget | null): node is HTMLElement =>
    node instanceof HTMLElement && node.dataset.minimotorUi === "true";
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (performance.now() - openedAt > FOCUS_GRACE_MS) return;
