...
let pending: { paint?: number } | null = null;

/** Attribute the next paint to `entry` — called by `recordLayout`, right after
 *  it pushes. */
export function armPaint(entry: { paint?: number }): void {
  pending = entry;
}
...
export function detachPaint(): void {
  pending = null;
}

/** The kit is drawing. Claims the next ordinal for the armed entry, once. */
export function notePaint(): void {
  const entry = pending;
  if (entry === null || entry.paint !== undefined) return;
  entry.paint = ++seq;
}

/** Start a fresh frame's numbering (and drop the armed entry with it). */
export function resetPaintSeq(): void {
  seq = 0;
  pending = null;
}
