...
import { type CharGrid, asGrid, cloneGrid, cols, glyphs, makeGrid, rows } from "./grid.js";
 ...
 *        #####`, { edge: true });
 */
export function analyze(sample: CharGrid | string, options: AnalyzeOptions = {}): TileModel {
  const grid = asGrid(sample);
  const width = cols(grid);
  const height = rows(grid);
  if (width === 0 || height === 0) throw new Error("Procgen.analyze: the sample is empty");
  ...
  if (wantEdge) tiles.push(OUTSIDE);
  const index = new Map(tiles.map((glyph, i) => [glyph, i]));
  const size = tiles.length;
  const weights = Array.from<number>({ length: size }).fill(0);
  const allowed = new Uint8Array(4 * size * size);

  /** Record that `b` may sit `dir`-ward of `a`, and the mirror of that fact. */
  const link = (dir: number, a: number, b: number): void => {
    allowed[(dir * size + a) * size + b] = 1;
    allowed[(((dir + 2) % 4) * size + b) * size + a] = 1;
  };

  const outside = wantEdge ? (index.get(OUTSIDE) as number) : -1;
  // Beyond the outside is more outside, so corners work out.
  if (outside >= 0) for (let dir = 0; dir < 4; dir++) link(dir, outside, outside);

  const scan = (source: CharGrid, countWeights: boolean): void => {
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const a = index.get(source[y][x]) as number;
        if (countWeights) weights[a]++;
        for (let dir = 0; dir < 4; dir++) {
          ...
          if (off && options.wrap) {
            ...
            if (outside >= 0) link(dir, a, outside);
            continue;
          }
          link(dir, a, index.get(source[ny][nx]) as number);
        }
      }
    }
  };
...
export function defineModel(spec: {
  ...
  const link = (d: number, ai: number, bi: number): void => {
  ...
  if (wantEdge) {
    const outside = index.get(OUTSIDE) as number;
    for (let dir = 0; dir < 4; dir++) link(dir, outside, outside);
    for (const glyph of spec.edge as readonly string[]) {
      const gi = index.get(glyph);
      if (gi === undefined) throw new Error(`Procgen.defineModel: unknown edge glyph "${glyph}"`);
      for (let dir = 0; dir < 4; dir++) link(dir, gi, outside);
    }
  }
  return {
    tiles,
 ...
 *  `mirror: true`. */
export function synthesize(model: TileModel, options: SynthesizeOptions): CharGrid {
  ...
  if (size === 0) throw new Error("Procgen.synthesize: the model has no tiles");
  if (options.weights && options.weights.length !== width * height * size) {
    throw new Error(
      `Procgen.synthesize: weights must hold cols*rows*${size} values, got ${options.weights.length}`,
    );
  }
  ...
  throw new Error(
    `Procgen.synthesize: no consistent layout after ${attempts} attempts — the model may be too sparse`,
  );
}

/** One collapse run. Returns null on a contradiction so the caller can retry. */
function collapse(model: TileModel, options: SynthesizeOptions, seed: number): CharGrid | null {
  const width = options.cols;
  const height = options.rows;
  const size = model.tiles.length;
  ...
  const weightAt = (cell: number, tile: number): number => {
  ...
  const heapC: number[] = [];
  const heapV: number[] = [];
  function heapPush(entropy: number, cell: number, stamp: number): void {
    let i = heapE.length;
    heapE.push(entropy);
    heapC.push(cell);
    heapV.push(stamp);
  ...
  function heapPop(): number {
    ...
    while (heapE.length > 0) {
      ...
      for (;;) {
        ...
        const r = l + 1;
        let small = i;
        if (l < heapE.length && heapE[l] < heapE[small]) small = l;
        if (r < heapE.length && heapE[r] < heapE[small]) small = r;
        if (small === i) break;
        swapHeap(small, i);
        i = small;
      }
      if (stamp === version[cell] && open[cell] > 1) return cell;
    }
    return -1;
  }
  function reheap(cell: number): void {
    if (open[cell] <= 1) return;
    ...
    const entropy =
      sumW[cell] > 0 ? Math.log(sumW[cell]) - sumWLogW[cell] / sumW[cell] : Math.log(open[cell]);
    heapPush(entropy - rng.random() * 1e-6, cell, version[cell]);
  }

  /** Rule out one tile at one cell. Returns false on a contradiction. */
  function remove(cell: number, tile: number): boolean {
    const at = cell * size + tile;
    if (!wave[at]) return true;
    wave[at] = 0;
    open[cell]--;
  ...
  function propagate(): boolean {
    while (stack.length > 0) {
      ...
      for (let dir = 0; dir < 4; dir++) {
        ...
        if (open[neighbour] === 1) continue;
        support.fill(0);
        for (let s = 0; s < size; s++) {
          if (!wave[cell * size + s]) continue;
          const row = (dir * size + s) * size;
          for (let t = 0; t < size; t++) if (allowed[row + t]) support[t] = 1;
        }
        for (let t = 0; t < size; t++) {
          if (!wave[neighbour * size + t] || support[t]) continue;
          if (!remove(neighbour, t)) return false;
        }
      }
    }
    return true;
  ...
  if (options.weights) {
    for (let cell = 0; cell < cells; cell++) {
      for (let t = 0; t < size; t++) {
        if (weightAt(cell, t) <= 0 && !remove(cell, t)) return null;
      }
    }
  }
  ...
  if (outside >= 0) {
    for (let cell = 0; cell < cells; cell++) {
      if (!remove(cell, outside)) return null;
    }
  }
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        for (let dir = 0; dir < 4; dir++) {
          ...
          if (nx >= 0 && ny >= 0 && nx < width && ny < height) continue;
          for (let t = 0; t < size; t++) {
            if (!wave[cell * size + t]) continue;
            if (!allowed[(dir * size + t) * size + outside] && !remove(cell, t)) return null;
          }
        }
      }
    }
  ...
  for (const [x, y, glyph] of options.fixed ?? []) {
    ...
    if (keep < 0) throw new Error(`Procgen.synthesize: fixed glyph "${glyph}" is not in the model`);
    const cell = y * width + x;
    for (let t = 0; t < size; t++) {
      if (t !== keep && !remove(cell, t)) return null;
    }
  }
  if (!propagate()) return null;

  for (let cell = 0; cell < cells; cell++) reheap(cell);
  ...
  for (;;) {
    ...
    for (let t = 0; t < size; t++) {
      if (t !== chosen && !remove(cell, t)) return null;
    }
    if (!propagate()) return null;
  }
...
export function resynthesize(
  grid: CharGrid,
  model: TileModel,
  patch: { x: number; y: number; w: number; h: number; seed?: number; attempts?: number },
): CharGrid {
  const width = cols(grid);
  const height = rows(grid);
  ...
  if (x1 <= x0 || y1 <= y0) return cloneGrid(grid);
  ...
  const fixed: Array<readonly [number, number, string]> = [];
  for (let y = gy0; y < gy1; y++) {
    for (let x = gx0; x < gx1; x++) {
      if (x >= x0 && x < x1 && y >= y0 && y < y1) continue;
      fixed.push([x - gx0, y - gy0, grid[y][x]] as const);
    }
  }
  ...
  for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) out[y][x] = patched[y - gy0][x - gx0];
  }
  return out;
}
