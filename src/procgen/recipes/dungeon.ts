...
import { type CharGrid, put } from "../grid.js";
import { type Room, rooms } from "../rooms.js";
 ...
 */
export function dungeon(options: DungeonOptions): DungeonResult {
  ...
  const steps: number[] = [];
  for (let i = 1; i < critical.length; i++) steps.push(i);
  shuffleInPlace(steps, rng);

  const usedKeyRooms = new Set<number>([shape.start, shape.end]);
  for (const step of steps) {
    if (locks.length >= wanted) break;
    ...
    locks.push({ between: [before, after] as const, at, keyRoom, keyAt });
  }
...
function nearestRoom(
  ...
  const linked = new Set<number>([from]);
  for (const [a, b] of links) {
    if (a === from) linked.add(b);
    if (b === from) linked.add(a);
  }
  let best = -1;
  ...
  for (let i = 0; i < placed.length; i++) {
    ...
    const dist =
      Math.abs(placed[i].cx - placed[from].cx) + Math.abs(placed[i].cy - placed[from].cy);
    if (dist < bestDist) {
      bestDist = dist;
      best = i;
    }
...
function doorCell(from: Room, to: Room): { x: number; y: number } {
...
function carveCorridor(grid: CharGrid, a: Room, b: Room, floor: string): void {
...
function shuffleInPlace(items: number[], rng: { integer(min: number, max: number): number }): void {
...
export const DUNGEON_MARKERS = { alsoWalkable: ["S", "E", "D", "k"] } as const;
