...
import { OUTSIDE, type TileModel } from "./wfc.js";
 ...
 *      );
 */
export function steer(model: TileModel, options: SteerOptions): SteerResult {
  ...
  if (size === 0) throw new Error("Procgen.steer: the model has no tiles");
  ...
  for (let t = 0; t < size; t++) if (t !== outside) placeable.push(t);
  if (placeable.length === 0) throw new Error("Procgen.steer: the model has no placeable tiles");
  ...
  for (let i = 0; i < illegal.length; i++) illegal[i] = model.allowed[i] ? 0 : 1;
  ...
  for (let cell = 0; cell < cells; cell++) {
    for (const t of placeable) field[cell * size + t] = start;
  }
  ...
  for (let step = 0; step < steps; step++) {
          ...
          for (let dir = 0; dir < 4; dir++) {
            ...
            for (const a of placeable) {
              ...
              for (const b of placeable) {
                if (illegal[row + b]) penalty += field[neighbour * size + b];
              }
    ...
    for (let cell = 0; cell < cells; cell++) {
      ...
      for (const t of placeable) field[cell * size + t] /= total;
    }
