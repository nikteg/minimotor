...
import { type CharGrid, cols, rows } from "./grid.js";
...
function walkableMask(grid: CharGrid, options: MetricOptions): Uint8Array {
  const width = cols(grid);
  const height = rows(grid);
  const walkable = new Set<string>([options.floor ?? ".", ...(options.alsoWalkable ?? [])]);
  const mask = new Uint8Array(width * height);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) mask[y * width + x] = walkable.has(grid[y][x]) ? 1 : 0;
  }
  return mask;
}
 ...
 *  thousands of times. */
function bfs(mask: Uint8Array, width: number, height: number, sources: number[]): Int32Array {
  const dist = new Int32Array(mask.length).fill(-1);
  const queue = new Int32Array(mask.length);
  let tail = 0;
  for (const source of sources) {
    if (source < 0 || source >= mask.length || !mask[source] || dist[source] >= 0) continue;
    dist[source] = 0;
    queue[tail++] = source;
  }
  for (let head = 0; head < tail; head++) {
    ...
    if (cx > 0) tail = visit(cell - 1, cell, dist, mask, queue, tail);
    if (cx < width - 1) tail = visit(cell + 1, cell, dist, mask, queue, tail);
    if (cy > 0) tail = visit(cell - width, cell, dist, mask, queue, tail);
    if (cy < height - 1) tail = visit(cell + width, cell, dist, mask, queue, tail);
  }
  return dist;
}
...
function visit(
  ...
  if (!mask[next] || dist[next] >= 0) return tail;
...
export function frequencies(grid: CharGrid): Record<string, number> {
  const total = cols(grid) * rows(grid);
  const counts: Record<string, number> = {};
  for (const row of grid) {
    for (const glyph of row) counts[glyph] = (counts[glyph] ?? 0) + 1;
  }
  for (const glyph of Object.keys(counts)) counts[glyph] /= total;
  return counts;
}
...
export function openness(grid: CharGrid, options: MetricOptions = {}): number {
  const mask = walkableMask(grid, options);
  let open = 0;
  for (let i = 0; i < mask.length; i++) open += mask[i];
...
export function reachableFraction(grid: CharGrid, options: MetricOptions = {}): number {
  const width = cols(grid);
  const height = rows(grid);
  const mask = walkableMask(grid, options);
  let total = 0;
...
export function longestPath(grid: CharGrid, options: MetricOptions = {}): number {
  const width = cols(grid);
  const height = rows(grid);
  const mask = walkableMask(grid, options);
  let first = -1;
  ...
  let far = first;
  for (let i = 0; i < away.length; i++) if (away[i] > away[far]) far = i;
  const back = bfs(mask, width, height, [far]);
  let longest = 0;
  for (let i = 0; i < back.length; i++) if (back[i] > longest) longest = back[i];
  return longest;
}
...
export function corridorRatio(grid: CharGrid, options: MetricOptions = {}): number {
  const width = cols(grid);
  const height = rows(grid);
  const mask = walkableMask(grid, options);
  let open = 0;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      open++;
      if (neighbours(mask, width, height, x, y) === 2) corridor++;
    }
  }
...
export function deadEnds(grid: CharGrid, options: MetricOptions = {}): number {
  const width = cols(grid);
  const height = rows(grid);
  const mask = walkableMask(grid, options);
  let count = 0;
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      if (mask[y * width + x] && neighbours(mask, width, height, x, y) === 1) count++;
    }
  }
  return count;
}

function neighbours(mask: Uint8Array, width: number, height: number, x: number, y: number): number {
  let open = 0;
  if (y > 0 && mask[(y - 1) * width + x]) open++;
  if (x < width - 1 && mask[y * width + x + 1]) open++;
  if (y < height - 1 && mask[(y + 1) * width + x]) open++;
  if (x > 0 && mask[y * width + x - 1]) open++;
  return open;
}
...
export function symmetry(grid: CharGrid): number {
  const width = cols(grid);
  const height = rows(grid);
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width >> 1; x++) {
      total++;
      if (grid[y][x] === grid[y][width - 1 - x]) same++;
    }
  }
 ...
 *  a fitness function. */
export function measure(grid: CharGrid, options: MetricOptions = {}): Metrics {
  return {
