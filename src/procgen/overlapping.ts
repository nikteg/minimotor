...
import { type CharGrid, asGrid, cols, makeGrid, rows } from "./grid.js";
...
export function overlapping(
  sample: CharGrid | string,
  options: OverlappingOptions = {},
): OverlappingModel {
  const grid = asGrid(sample);
  const width = cols(grid);
  const height = rows(grid);
  if (width === 0 || height === 0) throw new Error("Procgen.overlapping: the sample is empty");
  ...
  if (!periodic && (width < n || height < n)) {
    throw new Error(
      `Procgen.overlapping: a ${width}x${height} sample is smaller than the ${n}x${n} window ` +
  ...
  const keys: string[] = [];
  const patterns: CharGrid[] = [];
  const weights: number[] = [];
  const byKey = new Map<string, number>();

  const add = (window: CharGrid, count: number): void => {
  ...
  for (let y = 0; y < lastY; y++) {
    for (let x = 0; x < lastX; x++) {
      const window = Array.from({ length: n }, (_, dy) =>
        Array.from({ length: n }, (_, dx) => grid[(y + dy) % height][(x + dx) % width]),
      );
      ...
      for (const variant of transforms(window, symmetry)) add(variant, 1);
    }
  }
  ...
  if (found > maxPatterns) {
    throw new Error(
      `Procgen.overlapping: ${found} patterns exceeds maxPatterns ${maxPatterns} — ` +
    ...
    for (let a = 0; a < found; a++) {
      ...
      for (let dir = 0; dir < 4; dir++) {
        ...
        for (let b = 0; b < found && !supported; b++) {
          if (alive[b] && table[(dir * found + a) * found + b]) supported = true;
        }
        if (!supported) {
          alive[a] = 0;
          changed = true;
  ...
  const keep: number[] = [];
  for (let a = 0; a < found; a++) if (alive[a]) keep.push(a);
  const size = keep.length;
  if (size === 0) {
    throw new Error(
      "Procgen.overlapping: no pattern can tile the plane — " +
  ...
  const index = new Map(liveKeys.map((key, i) => [key, i] as const));

  const model: TileModel = { tiles: liveKeys, weights: liveWeights, allowed };
 ...
 */
export function glyphWeights(
  built: OverlappingModel,
  preference: Readonly<Record<string, number>>,
  ...
  const perPattern = built.patterns.map((pattern) => {
    ...
    for (const row of pattern) {
      for (const glyph of row) scale *= preference[glyph] ?? 1;
    }
  ...
  for (let cell = 0; cell < cells; cell++) {
    for (let t = 0; t < size; t++) weights[cell * size + t] = perPattern[t];
  }
  return weights;
}
...
function agrees(a: CharGrid, b: CharGrid, dx: number, dy: number, n: number): boolean {
  ...
  for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) {
      if (a[y][x] !== b[y - dy][x - dx]) return false;
    }
  }
  return true;
}
...
function transforms(window: CharGrid, symmetry: number): CharGrid[] {
  if (symmetry === 1) return [window];
  const mirror = (g: CharGrid) => g.map((row) => row.slice().reverse());
  const turn = (g: CharGrid) => g[0].map((_, x) => g.map((row) => row[x]).reverse());
  if (symmetry === 2) return [window, mirror(window)];
  const rotations = [window];
  for (let i = 1; i < 4; i++) rotations.push(turn(rotations[i - 1]));
