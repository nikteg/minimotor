// Module-local transition tests.
import { describe, it, expect, vi } from "vitest";
import { curtain, fade, wipe, run } from "@src/transitions/index.js";
...
describe("Transitions.run", () => {
  ...
  it("fires lifecycle phases in cover/swap/reveal order", () => {
    const order: string[] = [];
    const transition = run(fade(400), {
      beforeCover: () => order.push("before"),
      swap: () => order.push("swap"),
      afterReveal: () => order.push("after"),
    });
    expect(order).toEqual(["before"]);
    transition.advance(200);
    expect(order).toEqual(["before", "swap"]);
    transition.advance(200);
    expect(order).toEqual(["before", "swap", "after"]);
  });
});
...
describe("Transitions.curtain", () => {
  it("closes two panels toward the middle", () => {
    const ctx = mockCtx();
    curtain(400).render(ctx, 0.5, vp);
    expect(ctx.fillRect).toHaveBeenNthCalledWith(1, 0, 0, 200, 600);
    expect(ctx.fillRect).toHaveBeenNthCalledWith(2, 600, 0, 200, 600);
  });
});
