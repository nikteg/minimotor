...
function build(draw: () => void): { game: Runtime; canvas: HTMLCanvasElement } {
...
function focused(game: Runtime): string | null {
  begin(game.ctx);
  return focusedId();
}
...
describe("Tab traversal", () => {
  ...
  it("keeps the arrows with the game when a PRESS focuses the canvas, not Tab", async () => {
    // A game with a HUD button: clicking the world focuses the canvas, and if
    // that pulled focus into the button the player would lose the arrow keys.
    const { game, canvas } = build(() => {
      button({ label: "Menu", id: "menu", x: 0, y: 0, w: 80 });
    });
    tick();
    tick();
    ...
    expect(game.keys.down("ArrowRight")).toBe(true);

    // Tabbing (or a programmatic focus) still lands on the first widget — the
    // press only suppresses the focus event it caused, in its own task.
    await new Promise((resolve) => setTimeout(resolve, 0));
    canvas.dispatchEvent(new FocusEvent("focus"));
    tick();
    expect(focused(game)).toBe("menu");
  });
