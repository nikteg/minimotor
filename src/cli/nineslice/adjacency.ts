...
import { cellRect, sockets, type Grid, type Sockets } from "./tiles.js";
...
const isEmpty = (image: Pixels, rect: Rect): boolean => {
  for (let y = 0; y < rect.sh; y++) {
    for (let x = 0; x < rect.sw; x++) {
      if (image.data[((rect.sy + y) * image.width + rect.sx + x) * 4 + 3] !== 0) return false;
    }
  }
  return true;
};
...
export function inferAdjacency(
  ...
  for (let row = 0; row < size.rows; row++) {
    for (let column = 0; column < size.cols; column++) {
      ...
      if (rect.sx + rect.sw > image.width || rect.sy + rect.sh > image.height) continue;
      if (isEmpty(image, rect)) continue;
  ...
  const set = (dir: number, a: number, b: number) => {
  ...
  for (let a = 0; a < n; a++) {
    for (let b = 0; b < n; b++) {
      ...
      if (nodes[b].sockets.south === nodes[a].sockets.north) set(0, a, b);
      if (nodes[a].sockets.east === nodes[b].sockets.west) set(1, a, b);
      if (nodes[a].sockets.south === nodes[b].sockets.north) set(2, a, b);
      if (nodes[b].sockets.east === nodes[a].sockets.west) set(3, a, b);
    }
  }

  const count = (side: keyof Sockets) => new Set(nodes.map((node) => node.sockets[side])).size;
  const pairs = Math.max(1, n * n);
  const density = {} as Record<Direction, number>;
  DIRECTIONS.forEach((direction, dir) => {
    let total = 0;
    for (let i = 0; i < n * n; i++) total += allowed[dir * n * n + i];
...
const OPPOSITE: Record<Direction, keyof Sockets> = {
  up: "north",
  right: "east",
  down: "south",
  left: "west",
...
export function adjacencyFindings(graph: TileGraph, name = "sheet"): Finding[] {
  ...
  if (sparse) {
    findings.push({
      ...
      message:
        `${n} tiles, but under 2% of tile pairs share a facing edge exactly ` +
  ...
  for (const direction of DIRECTIONS) {
    ...
    for (let a = 0; a < n; a++) {
      let any = false;
      for (let b = 0; b < n && !any; b++) any = graph.allowed[(dir * n + a) * n + b] === 1;
      if (!any) dead.push(`${graph.nodes[a].column},${graph.nodes[a].row}`);
    }
    ...
    findings.push({
      ...
      code: "dead-side",
      message:
        `${dead.length} tile(s) present a ${OPPOSITE[direction]} edge no other tile in the sheet ` +
...
export function toTileModel(graph: TileGraph): {
  tiles: string[];
  weights: number[];
  allowed: Uint8Array;
} {
