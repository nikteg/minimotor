...
const sample = (line: Uint8Array, index: number, depth: number): number => {
...
export function decodePng(file: Uint8Array): Pixels {
  ...
  while (offset + 8 <= file.length) {
    ...
    if (type === "IHDR") {
      ...
      depth = body[8];
      colorType = body[9];
      if (body[12] !== 0) throw new Error("interlaced (Adam7) PNG is not supported");
  ...
  const raw = unfilter(
    new Uint8Array(inflateSync(merged)),
    height,
    stride,
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      const read = (channel: number) => sample(line, x * channels + channel, depth);
      if (colorType === 3) {
        const index = read(0);
