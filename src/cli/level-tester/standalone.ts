...
export async function startStandaloneLevelTester(
  options: StandaloneLevelTesterOptions,
): Promise<StandaloneLevelTester> {
  ...
  if (!address || typeof address === "string") throw new Error("level tester has no TCP address");
  const displayHost = host === "0.0.0.0" || host === "::" ? "localhost" : host;
  return {
    server,
    url: `http://${displayHost}:${address.port}/`,
    async close() {
      for (const client of tester.clients) client.close();
      tester.close();
