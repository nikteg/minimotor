...
import { array, constArray, identifier, property, unionType } from "@src/cli/utils.js";
...
interface LDtkLevel {
  ...
  layerInstances?: { __identifier?: string; entityInstances?: LDtkEntityInstance[] }[] | null;
}
...
export function readProject(inputPath: string): LDtkProject {
  const project = JSON.parse(readFileSync(inputPath, "utf8")) as LDtkProject;
  const loadLevel = (level: LDtkLevel) => {
    if (level.layerInstances || !level.externalRelPath) return level;
    return JSON.parse(
      readFileSync(resolve(dirname(inputPath), level.externalRelPath), "utf8"),
    ) as LDtkLevel;
  };
  return {
    ...project,
...
let _schemaJson: object | undefined;
let _schemaValidate: ReturnType<InstanceType<typeof Ajv>["compile"]> | undefined;

function getSchemaValidator() {
  if (_schemaValidate) return _schemaValidate;
  if (!_schemaJson) {
    // Vite represents source modules with an HTTP URL in tests; the installed
    // CLI uses a file URL beside the copied schema.
    const path =
      schemaPath.protocol === "file:"
  ...
  const schema = { ...(_schemaJson as Record<string, unknown>) };
  ...
  const otherTypes = schema.otherTypes as Record<string, Record<string, unknown>>;
  const level = otherTypes.Level;
  const properties = level.properties as Record<string, unknown>;
  properties.customFields = { type: "object" };
  _schemaValidate = ajv.compile(schema as AnySchema);
  return _schemaValidate;
}

/** Validate the project JSON against the LDtk 1.5.3 schema. */
function validateSchema(project: unknown): string[] {
  const validate = getSchemaValidator();
  const valid = validate(project);
  if (valid) return [];
  return (validate.errors ?? []).map(
    (error) => `[schema] ${error.instancePath || "(root)"}: ${error.message}`,
  );
}

/** Validate the conventions consumed by `LDtk.world`, plus JSON schema. */
export function checkLDtk(project: LDtkProject): LDtkCheckResult {
  const errors: string[] = validateSchema(project);
  ...
  const ids = new Map<string, { level: string; type: string; fields: Map<string, unknown> }>();

  for (const level of levels) {
    if (levelNames.has(level.identifier)) errors.push(`duplicate level "${level.identifier}"`);
    levelNames.add(level.identifier);
    const layers = level.layerInstances ?? [];
    const names = new Set(layers.map((layer) => layer.__identifier));
    if (!names.has("World")) warnings.push(`${level.identifier}: no "World" collision layer`);
    if (!names.has("Art")) warnings.push(`${level.identifier}: no "Art" tile layer`);
    for (const layer of layers) {
      for (const entity of layer.entityInstances ?? []) {
        if (!definitionNames.has(entity.__identifier)) {
          errors.push(`${level.identifier}: undefined entity "${entity.__identifier}"`);
        }
        if (!entity.iid) continue;
        if (ids.has(entity.iid)) errors.push(`duplicate entity IID "${entity.iid}"`);
        ids.set(entity.iid, {
  ...
  for (const entity of ids.values()) {
    if (!portalTypes.has(entity.type)) continue;
    const ref = entity.fields.get("To") as { entityIid?: unknown } | null | undefined;
    ...
    const destinations = links.get(entity.level) ?? new Set<string>();
...
function fieldType(field: LDtkField, enums: { has(value: string): boolean }): string {
  const raw = field.type ?? "";
  let type: string;
  if (raw === "F_Int" || raw === "F_Float") type = "number";
  else if (raw === "F_Bool") type = "boolean";
  else if (raw === "F_EntityRef") type = "LDtkEntityRef";
  else if (raw === "F_Point") type = "LDtkGridPoint";
  else if (raw === "F_Tile") type = "LDtkTileRef";
  else if (raw.startsWith("F_Enum(")) {
    const name = raw.slice(7, -1);
    type = enums.has(name) ? identifier(name) : "string";
  } else type = "string";
  if (field.isArray) type = `readonly ${type}[]`;
  if (field.canBeNull) type += " | null";
  return type;
}

/** Generate the TypeScript companion for an LDtk project. */
export function generateLDtkTypes(
  project: LDtkProject,
  sourceName = "project.ldtk",
  ...
  const enumDefs = new Map(
    (project.defs.enums ?? []).map((entry) => [entry.identifier, entry] as const),
  );
    ...
    projectLevels.map(
      (level) =>
        [
          ...
          [
            ...new Set(
              (level.layerInstances ?? []).flatMap((layer) =>
                (layer.entityInstances ?? []).map((entity) => entity.__identifier),
              ),
            ),
          ].sort(),
        ] as const,
  ...
  const tilesets = (project.defs.tilesets ?? []).filter(
    (tileset): tileset is LDtkTileset & { relPath: string } => !!tileset.relPath,
  );
  const artTilesetUid = project.defs.layers?.find(
    (layer) => layer.identifier === "Art",
  )?.tilesetDefUid;
  const artTileset =
    tilesets.find((tileset) => tileset.uid === artTilesetUid) ??
    (tilesets.length === 1 ? tilesets[0] : undefined);
  const assetKey = (tileset: LDtkTileset) =>
    tileset === artTileset
      ? "terrain"
  ...
  for (const [name, definition] of enumDefs) {
    code += unionType(
      identifier(name),
      (definition.values ?? []).map((value) => value.id),
    );
  }
  if (enumDefs.size) code += "\n";
  ...
  for (const tileset of tilesets) {
    code += `  ${property(assetKey(tileset))}: new URL(${JSON.stringify(
      `${sourceDirectory}${tileset.relPath}`,
...
function parseTypesArgs(args: string[]): TypesOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "-o" || arg === "--out") {
      output = args[++i];
      if (!output) throw new Error(`${arg} needs a file`);
  ...
  if (!input) throw new Error("missing <project.ldtk>");
  if (stdout && output) throw new Error("--stdout cannot be combined with --out");
...
function runTypes(args: string[]): void {
  ...
  const code = generateLDtkTypes(
    source,
    label,
    sourceUrl.startsWith(".") ? sourceUrl : `./${sourceUrl}`,
  );
  if (options.stdout) {
...
function runCheck(args: string[]): void {
  ...
  process.stdout.write(
    `valid: ${relative(process.cwd(), input)} (${result.warnings.length} warnings)\n`,
  );
}
...
function runWatch(args: string[]): void {
  ...
  watch(dirname(options.inputPath), { recursive: true }, (_event, file) => {
    if (file && !file.endsWith(".ldtk") && !file.endsWith(".ldtkl")) return;
    clearTimeout(timer);
...
export default defineFeature({
  ...
  run(args) {
    ...
    if (args[0] === "types") runTypes(args.slice(1));
    else if (args[0] === "check") runCheck(args.slice(1));
    else if (args[0] === "watch") runWatch(args.slice(1));
    else throw new Error(`unknown ldtk command "${args.join(" ")}"\n\n${help}`);
  },
});
