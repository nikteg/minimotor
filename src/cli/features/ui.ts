...
import {
  ...
  type RegionInput,
  type RegionReport,
} from "@src/cli/nineslice/analyze.js";
import { decodePng, encodePng, type Pixels } from "@src/cli/nineslice/png.js";
import { analyzeAutotile, analyzeTileFrame, type Grid } from "@src/cli/nineslice/tiles.js";
import { adjacencyFindings, DIRECTIONS, inferAdjacency } from "@src/cli/nineslice/adjacency.js";
import { annotate, formatRegion, pixelMap, previewSheet, rank } from "@src/cli/nineslice/report.js";
import { takeFlag, takeOption } from "@src/cli/utils.js";
...
const numbers = (raw: string, count: number, flag: string): number[] => {
...
const toRect = (value: number[]): Rect => ({
  sx: value[0],
  sy: value[1],
  sw: value[2],
  sh: value[3],
...
const toInsets = (value: number[]): Insets => ({
  left: value[0],
  top: value[1],
  right: value[2],
  bottom: value[3],
...
interface ManifestRegion {
  /** TilesetSkinManifest spells the rect out; the terse form takes an array. */
  x?: number;
  y?: number;
  w?: number;
  h?: number;
...
interface Manifest {
  /** `image` is what TilesetSkinManifest calls it; `atlas` is accepted too. */
  image?: string;
  atlas?: string;
  frames?: Record<string, ManifestRegion>;
  regions?: Record<string, ManifestRegion>;
}

const asRect = (region: ManifestRegion, name: string): Rect => {
  if (Array.isArray(region.rect)) return toRect(numbers(region.rect.join(","), 4, `${name}.rect`));
  if (region.rect && typeof region.rect === "object") return region.rect;
...
const asInsets = (value: ManifestRegion["insets"]): Insets | undefined => {
  if (Array.isArray(value)) return value.length === 4 ? toInsets(value) : undefined;
...
function fromManifest(path: string): { atlas: string; regions: RegionInput[] } {
  const manifest = JSON.parse(readFileSync(path, "utf8")) as Manifest;
  const image = manifest.image ?? manifest.atlas;
  if (!image) throw new Error(`${path} has no "image"`);
  const regions = Object.entries(manifest.frames ?? manifest.regions ?? {}).map(
    ([name, region]) => ({
      name,
  ...
  if (!regions.length) throw new Error(`${path} declares no frames`);
...
interface FrameLike {
  ...
  sw?: unknown;
  sh?: unknown;
  insets?: { left?: unknown; top?: unknown; right?: unknown; bottom?: unknown };
}

const isFrame = (value: unknown): value is Required<FrameLike> => {
  const frame = value as FrameLike | null;
  return (
    !!frame &&
    typeof frame === "object" &&
    ["sx", "sy", "sw", "sh"].every((key) => typeof frame[key as keyof FrameLike] === "number") &&
    !!frame.insets &&
    ["left", "top", "right", "bottom"].every(
      (key) => typeof frame.insets![key as keyof FrameLike["insets"]] === "number",
    )
  );
};
...
function harvest(value: unknown, path: string, into: Map<string, RegionInput>, depth = 0): void {
  if (!value || typeof value !== "object" || depth > 6) return;
  if (isFrame(value)) {
    const frame = value as unknown as {
      sx: number;
      sy: number;
      sw: number;
      sh: number;
    ...
    if (existing) existing.name = `${existing.name} ${path}`;
    else {
      into.set(key, {
        name: path,
  ...
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    if (key === "image" || key === "mapping") continue;
...
async function fromModule(path: string, atlas: Pixels): Promise<RegionInput[]> {
  const stub = {
    width: atlas.width,
    height: atlas.height,
  ...
  try {
    module = (await import(pathToFileURL(resolve(path)).href)) as Record<string, unknown>;
  } catch (error) {
    throw new Error(
      `could not import ${path}: ${error instanceof Error ? error.message : String(error)}\n` +
        "Theme modules import the built package — run `pnpm build` first, or use a JSON manifest.",
    );
  }
  const found = new Map<string, RegionInput>();
  for (const [name, exported] of Object.entries(module)) {
    if (typeof exported !== "function" || name.startsWith("load")) continue;
    let produced: unknown;
    try {
      produced = (exported as (image: unknown) => unknown)(stub);
    } catch {
  ...
  if (!found.size) throw new Error(`${path} exported no nine-slice frames`);
  return [...found.values()];
}
...
function writeImages(directory: string, atlas: Pixels, report: RegionReport): string[] {
...
async function nineslice(args: string[]): Promise<void> {
  ...
  if (!input) throw new Error(`nineslice needs an atlas, manifest, or theme module\n\n${help}`);
  ...
  if (input.endsWith(".json")) {
    ...
    if (!atlasOption) throw new Error("a theme module needs --atlas <atlas.png>");
    atlasPath = resolve(atlasOption);
    deferred = resolve(input);
  } else {
    atlasPath = resolve(input);
    if (!rectOption) throw new Error("checking an atlas directly needs --rect <x,y,w,h>");
    regions = [
      {
        name: "rect",
  ...
  if (deferred) regions = await fromModule(deferred, atlas);
  if (only.length) {
    regions = regions.filter((region) =>
      only.some((filter) => region.name.toLowerCase().includes(filter.toLowerCase())),
    );
    if (!regions.length) throw new Error(`--only matched no frames`);
  }
  ...
  for (const report of reports) {
    ...
    if (breaks.top.length || breaks.left.length) {
      report.findings.push({
        ...
        code: "composed-break",
        message:
          `composed at ${SIZES[1].w}×${SIZES[1].h} the pattern breaks phase at ` +
  ...
  const failed = findings.some(
    (finding) => finding.level === "error" || (strict && finding.level === "warning"),
  );
  if (failed) throw new Error("nine-slice verification failed");
}

const parseGrid = (args: string[]): Grid => {
  const raw = takeOption(args, "--grid");
  if (!raw) throw new Error("this command needs --grid <x,y,tileW,tileH[,spacing]>");
  const parts = raw.split(",").map((part) => Number(part.trim()));
...
function emit(findings: Finding[], json: boolean, strict: boolean, headline: string): void {
...
function frame(args: string[]): void {
  const json = takeFlag(args, "--json");
  const strict = takeFlag(args, "--strict");
  const grid = parseGrid(args);
  const input = args.shift();
  if (!input) throw new Error("frame needs an atlas");
  if (args.length) throw new Error(`unknown option "${args[0]}"`);
  const atlas = decodePng(readFileSync(resolve(input)));
  emit(
    analyzeTileFrame(atlas, grid),
    json,
    strict,
    `${resolve(input)} (${atlas.width}×${atlas.height}), 3×3 frame of ${grid.tile.w}×${grid.tile.h} tiles at ${grid.x},${grid.y}`,
  );
}

function autotile(args: string[]): void {
  const json = takeFlag(args, "--json");
  const strict = takeFlag(args, "--strict");
  const grid = parseGrid(args);
  ...
  const input = args.shift();
  if (!input) throw new Error("autotile needs an atlas");
  if (args.length) throw new Error(`unknown option "${args[0]}"`);
  const atlas = decodePng(readFileSync(resolve(input)));
  ...
  if (!Number.isInteger(cols) || cols < 1) throw new Error("--cols needs a positive whole number");
  if (!Number.isInteger(rows) || rows < 1) throw new Error("--rows needs a positive whole number");
...
export default defineFeature({
  ...
  async run(input) {
    ...
    if (input[0] === "nineslice") await nineslice(args);
    else if (input[0] === "frame") frame(args);
    else if (input[0] === "autotile") autotile(args);
    else throw new Error(`unknown ui command "${input[0]}"\n\n${help}`);
  },
});
