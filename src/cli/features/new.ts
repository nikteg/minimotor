...
import { files, takeFlag } from "@src/cli/utils.js";
...
function ownVersion(): string {
  return JSON.parse(readFileSync(resolve(minimotorRoot, "package.json"), "utf8")).version as string;
}

/** Is this CLI running out of a source checkout rather than an installed copy? */
export function runningFromCheckout(): boolean {
 ...
 */
export function minimotorDependency(projectRoot: string, link = false): string {
  if (!link) return `^${ownVersion()}`;
  const path = relative(projectRoot, minimotorRoot).split(sep).join("/");
  if (path === "") return "link:."; // scaffolded into the checkout itself
  ...
  if (climbs > 6) return `link:${minimotorRoot.split(sep).join("/")}`;
...
export function templateFiles(
  template: string,
  name: string,
  minimotor = minimotorDependency(process.cwd(), false),
  ...
  const fill = (source: string) =>
    source.split("{{name}}").join(name).split("{{minimotor}}").join(minimotor);
  ...
  run(input) {
    ...
    if (!link && runningFromCheckout()) {
      process.stdout.write(
        `note: depends on the published minimotor@${minimotor}; pass --link to use this checkout\n`,
      );
    }
