...
import type { LadderSource } from "@src/collision/index.js";
...
export const ground: TileSpec = { solid: true };

/** A one-way platform: land on it from above, jump up through it. */
export const platform: TileSpec = { solid: true, oneWay: true };
 ...
 */
export function tagged(tag: string, extra: TileSpec = {}): TileSpec {
...
const views = new WeakMap<Level, Map<string, LadderSource>>();
...
export function climbable(level: Level, tag: string = LADDER): LadderSource {
  let byTag = views.get(level);
  if (!byTag) views.set(level, (byTag = new Map()));
  let view = byTag.get(tag);
  if (!view) {
    byTag.set(
      tag,
      (view = {
        laddersNear: (area: Rect, out: Rect[]): Rect[] => level.rectsNear(tag, area, out),
      }),
    );
  }
  return view;
