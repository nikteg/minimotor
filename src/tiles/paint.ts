...
export function createPainter<C extends string>(
  dims: GridDims,
  cells: string[][],
  level: Level<C>,
): Painter<C> {
  ...
  function dualFilled(cx: number, cy: number, char: string, connect: "same" | "solid"): boolean {
    if (connect === "solid") {
  ...
  function paintDual(
    ...
    for (let cy = y0; cy <= y1 + 1; cy++) {
      for (let cx = x0; cx <= x1 + 1; cx++) {
        const mask =
          (dualFilled(cx - 1, cy - 1, char, connect) ? 1 : 0) |
