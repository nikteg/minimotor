...
const NUMBERS = ["x", "y", "vx", "vy", "w", "h", "rot", "spin", "facing"] as const;
...
const maxSize = (state: BodySnapshot): number =>
  2 +
  NUMBERS.length * 4 +
  3 +
...
export function writeBodySnapshot(view: DataView, at: number, state: BodySnapshot): number {
  ...
  if (typeof state.grounded === "boolean") {
    mask |= GROUNDED_SET;
    if (state.grounded) mask |= GROUNDED_VALUE;
  }
  if (typeof state.active === "boolean") {
    mask |= ACTIVE_SET;
    if (state.active) mask |= ACTIVE_VALUE;
  }
...
export function readBodySnapshot(
  view: DataView,
  at: number,
): { state: BodySnapshot; at: number } | null {
  ...
  if (mask & GROUNDED_SET) state.grounded = (mask & GROUNDED_VALUE) !== 0;
  if (mask & ACTIVE_SET) state.active = (mask & ACTIVE_VALUE) !== 0;
  ...
  return { state: state as unknown as BodySnapshot, at: cursor };
}
...
export interface SyncCodec<T> {
  ...
  decode(bytes: Uint8Array): { state: T; sentAt: number } | null;
}

/** Packed codec for a single replicated body. */
export function bodyCodec<T extends BodySnapshot>(): SyncCodec<T> {
  return {
    tag: "b",
    encode(state, sentAt) {
    ...
    decode(bytes) {
      if (bytes.length < STAMP + 2) return null;
      const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.length);
      const sentAt = view.getFloat64(0, true);
      const read = readBodySnapshot(view, STAMP);
      return read && { state: read.state as T, sentAt };
    },
  };
}

/** Packed codec for a keyed collection of bodies (`syncBodies`). */
export function bodiesCodec<T extends BodySnapshot>(): SyncCodec<Array<{ id: string; state: T }>> {
  return {
    tag: "bs",
    encode(entities, sentAt) {
      let size = LIST_HEADER;
      for (const entity of entities) size += 1 + entity.id.length * 3 + maxSize(entity.state);
