 ...
 *  that fails to boot. */
export function alphaMap(image: FontImage): { data: Uint8ClampedArray; w: number } | null {
  ...
  try {
    ...
    for (let i = 3; i < pixels.length; i += 4) if (pixels[i] !== 0) return { data: pixels, w };
    return null;
  } catch {
    return null;
  }
...
function inkSpan(
  alpha: { data: Uint8ClampedArray; w: number },
  x0: number,
  y0: number,
  cw: number,
  ch: number,
...
export function sliceGrid(
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (sx + grid.cellW > image.width || sy + grid.cellH > image.height) break;
