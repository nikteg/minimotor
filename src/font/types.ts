...
import type { TextHAlign, TextVAlign } from "@src/engine/text.js";

/** An image with known dimensions — a loaded `<img>`, a canvas, an
 *  `ImageBitmap`. The same shape `Sprites.contentBounds` takes. */
export type FontImage = CanvasImageSource & { width: number; height: number };
...
export interface BitmapFont {
  ...
  measureBlock(str: string): { w: number; h: number };
