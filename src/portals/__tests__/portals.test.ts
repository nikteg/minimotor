// Module-local portal tests.
import { describe, expect, it, vi } from "vitest";
import { createPortalRouter } from "@src/portals/index.js";
import { world } from "@src/tiles/index.js";
import type { GoOptions } from "@src/scenes/index.js";
...
function immediateGo(spy?: ReturnType<typeof vi.fn>) {
  return (name: string, options: GoOptions = {}) => {
    options.beforeCover?.();
    options.onSwap?.();
    options.afterReveal?.();
    spy?.(name, options);
...
describe("Portals", () => {
  it("changes area, places the body, stops motion, and navigates scenes", () => {
    ...
    const portals = createPortalRouter({
      body,
      scenes: { go: immediateGo(go), active: "outside" },
      auto: false,
      areas: {
        field: {
          scene: "outside",
  ...
  it("disarms paired doors until the body leaves the destination trigger", () => {
    ...
    const router = createPortalRouter({
      body,
      scenes: { go: immediateGo(), active: "play" },
      auto: false,
      areas: {
        a: {
          scene: "play",
  ...
  it("uses an ordinary tile-string world directly", () => {
    ...
    const router = createPortalRouter({
      body,
      scenes: { go: immediateGo(go), active: "game" },
      world: maps,
      scene: "game",
      auto: false,
    });
