...
import { STEP_MS, type ClockHandle } from "../clock.js";
...
export interface ParticleOptions {
  ...
  clock: ClockHandle;
  /** Milliseconds per simulation step — an app's `Loop.step`. Defaults to the
   *  standard rate; `createParticles(app)` passes the app's own. */
  step?: number;
...
export function createParticleSystem(options: ParticleOptions): ParticleSystem {
  ...
  const stepMs = options.step ?? STEP_MS;
  const live: Particle[] = [];
  ...
  function fold(): void {
    const now = clock.now;
    let steps = Math.floor((now - lastMs) / stepMs);
    if (steps <= 0) return;
    lastMs += steps * stepMs;
