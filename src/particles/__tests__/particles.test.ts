// Module-local particle tests.
import { describe, it, expect, vi } from "vitest";
import { createParticleSystem } from "@src/particles/index.js";
...
function sys(rng: () => number, t: ReturnType<typeof stepper>) {
  return createParticleSystem({ rng, clock: createClockHandle(1000 / 60, t.steps) });
}
...
describe("Particles.create", () => {
  ...
  it("emit() is chance-gated, one particle per call", () => {
    const t = stepper();
    let roll = 0;
    const p = createParticleSystem({
      rng: () => roll,
      clock: createClockHandle(1000 / 60, t.steps),
    });
    roll = 0.9;
