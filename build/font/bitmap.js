...
import { tint } from "../sprites/raster.js";
...
function build(image, table, size, cellW, options) {
    ...
    for (const [ch, glyph] of table) {
        ...
        if (forced !== undefined)
            glyph.advance = forced;
    }
    const fallback = options.fallback !== undefined ? table.get(options.fallback) : undefined;
    const glyph = (ch) => table.get(ch) ?? fallback;
    const advanceOf = (ch, extra) => {
        const g = glyph(ch);
        return (g ? g.advance : space) + extra;
    };
    const measureLine = (str, extra) => {
        let w = 0;
        for (const ch of str)
            w += advanceOf(ch, extra);
    ...
    const font = {
        ...
        measureBlock(str) {
            ...
            for (const line of lines)
                w = Math.max(w, measureLine(line, tracking));
            ...
            for (const pass of passes) {
                for (let i = 0; i < lines.length; i++) {
                    ...
                    for (const ch of line) {
                        ...
                        if (g.sw > 0 && g.sh > 0) {
                            blitPixelAligned(ctx, pass.src, g.sx, g.sy, g.sw, g.sh, pen + g.ox * scale, lineTop + g.oy * scale, g.sw * scale, g.sh * scale);
                        }
...
export function atlas(image, options) {
    ...
    if (!(cellW > 0) || !(cellH > 0))
        throw new Error("Font.atlas: cell must be a positive size");
    const gap = options.gap ?? 0;
    const originX = options.origin?.x ?? 0;
    const originY = options.origin?.y ?? 0;
    const cols = options.cols ?? Math.max(1, Math.floor((image.width - originX + gap) / (cellW + gap)));
    const chars = options.chars ?? ASCII;
    const table = sliceGrid(image, chars, { cellW, cellH, cols, originX, originY, gap }, options.trim ?? true);
    if (table.size === 0) {
        throw new Error(`Font.atlas: no glyph fits — a ${cellW}x${cellH} cell at (${originX}, ${originY}) ` +
 ...
 *  `advance` or offset. */
export function glyphs(image, options) {
    const table = new Map();
    let tallest = 0;
    let widest = 0;
    for (const [ch, spec] of Object.entries(options.glyphs)) {
        const g = Array.isArray(spec)
    ...
    if (table.size === 0)
        throw new Error("Font.glyphs: at least one glyph is required");
