...
export function affineFromDom(m) {
    return { a: m.a, b: m.b, c: m.c, d: m.d, e: m.e, f: m.f };
}
export function copyAffine(m, out = { a: 1, b: 0, c: 0, d: 1, e: 0, f: 0 }) {
    out.a = m.a;
    out.b = m.b;
    out.c = m.c;
    out.d = m.d;
...
export function multiplyAffine(m, n, out) {
...
export function transformPoint(m, x, y, out = scratch) {
...
export function snapDest(m, x, y, w, h) {
    if (!isAxisAligned(m))
        return { x, y, w, h };
...
export function scissorFromRect(m, x, y, w, h, canvasW, canvasH) {
...
export function toClipSpace(x, y, canvasW, canvasH) {
    const w = canvasW || 1;
    const h = canvasH || 1;
    return { x: (x / w) * 2 - 1, y: 1 - (y / h) * 2 };
}
function writeVertex(buf, o, x, y, u, v, r, g, b, a, canvasW, canvasH) {
...
export function writeQuad(buf, quad, m, x, y, w, h, u0, v0, u1, v1, r, g, b, a, canvasW, canvasH) {
...
export function writeQuadCorners(buf, quad, m, corners, u0, v0, u1, v1, r, g, b, a, canvasW, canvasH) {
    const o = quad * FLOATS_PER_QUAD;
    const uvs = [
        u0,
        v0,
...
export function spriteCorners(x, y, w, h, ax, ay, rot, scale, flipX, flipY) {
...
export function sortSpritesByZAndTexture(list) {
    const ids = new Map();
    let next = 0;
    const texId = (img) => {
        let id = ids.get(img);
...
export function prepareSprites(list, scratch) {
    scratch.length = 0;
    for (const s of list)
        scratch.push(s);
...
export function resolveSprite(s, lerp, view) {
    if (s.visible === false)
        return null;
...
export function readTransform(ctx) {
