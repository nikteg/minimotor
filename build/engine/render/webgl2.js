...
import { FLOATS_PER_QUAD, IDENTITY, copyAffine, imageSize, prepareSprites, resolveSprite, scissorFromRect, spriteCorners, writeQuad, writeQuadCorners, } from "./math.js";
...
function compile(gl, type, src) {
    ...
    if (!sh)
        throw new Error("Minimotor: WebGL2 createShader failed");
...
function link(gl, vs, fs) {
    ...
    if (!prog)
        throw new Error("Minimotor: WebGL2 createProgram failed");
...
function stackUnder(overlay, layer) {
...
export function createWebGL2Renderer(overlay, opts = {}) {
    ...
    if (!raw) {
        if (opts.required) {
            throw new Error('Minimotor: WebGL2 is required (renderer: "webgl") but getContext("webgl2") returned null');
        }
        return null;
    }
    const gl = raw;
    ...
    if (!vao || !vbo || !ibo)
        throw new Error("Minimotor: WebGL2 buffer allocation failed");
    ...
    if (!whiteTex)
        throw new Error("Minimotor: WebGL2 texture allocation failed");
    ...
    let destroyed = false;
    let clipOn = false;
    const clearColor = opts.background ? parseRgba(opts.background) : [0, 0, 0, 0];
    ...
    function useTexture(tex) {
        if (bound !== tex && quadCount > 0)
            flush();
        bound = tex;
    }
    function isLiveCanvas(image) {
    ...
    function pushQuad(tex, dx, dy, dw, dh, u0, v0, u1, v1, r, g, b, a, corners) {
        if (quadCount >= MAX_QUADS)
            flush();
        useTexture(tex);
        if (corners) {
            writeQuadCorners(verts, quadCount, transform, corners, u0, v0, u1, v1, r, g, b, a, canvasW, canvasH);
        }
        else {
            writeQuad(verts, quadCount, transform, dx, dy, dw, dh, u0, v0, u1, v1, r, g, b, a, canvasW, canvasH);
        }
        quadCount += 1;
    }
    const renderer = {
        ...
        sprites(list, opts = {}) {
            ...
            for (const s of ordered) {
                ...
                if (rotated) {
                    pushQuad(tex, 0, 0, 0, 0, u0, v0, u1, v1, 1, 1, 1, r.alpha, spriteCorners(r.x, r.y, r.w, r.h, r.ax, r.ay, r.rot, r.scale, r.flipX, r.flipY));
                }
                else {
                    pushQuad(tex, r.x - r.ax * r.w, r.y - r.ay * r.h, r.w, r.h, u0, v0, u1, v1, 1, 1, 1, r.alpha);
                }
            }
        },
        blitImage(image, sx, sy, sw, sh, dx, dy, dw, dh, alpha = 1, tint) {
            ...
            pushQuad(tex, dx, dy, dw, dh, sx / iw, sy / ih, (sx + sw) / iw, (sy + sh) / ih, tr, tg, tb, ta);
        },
        fillQuad(dx, dy, dw, dh, rgba) {
            if (destroyed || rgba[3] <= 0)
                return;
