// ---------- On-screen touch control implementation ----------
...
import { Buttons, createGamepadTracker, } from "../input/index.js";
...
export function padButtonIndex(button) {
...
export function computeStick(dx, dy, radius, deadzone = 0) {
...
const maxMag = (a, b) => (Math.abs(a) >= Math.abs(b) ? a : b);
...
export function fuseGamepad(touch, hw) {
    if (!hw || !hw.connected)
        return touch;
    ...
    for (let i = 0; i < nAxes; i++)
        axes[i] = maxMag(touch.axes[i] ?? 0, hw.axes[i] ?? 0);
    return { connected: true, buttons, axes };
}
const registry = new WeakMap();
const now = () => (typeof performance !== "undefined" ? performance.now() : 0);
function readHardware(index) {
    if (index === null)
        return null;
...
function synthFromTouch(st) {
    const buttons = [];
    for (let i = 0; i < STANDARD_BUTTONS; i++)
        buttons[i] = { pressed: false };
    for (const target of st.pointers.values()) {
        if (target.kind !== "button" || target.spec.button === undefined)
            continue;
...
function anchorCenter(st, anchor) {
    const { w, h } = windowSize(st);
    return {
...
function buttonBounds(st, button) {
...
// ---------- Haptics ----------
function fireHaptics(cfg, spec, phase) {
    const h = spec.haptics ?? cfg.haptics;
    if (!h)
        return;
    const hc = h === true ? {} : h;
    if (phase === "press" && hc.onPress === false)
        return;
    if (phase === "release" && !(hc.onRelease ?? false))
        return;
...
function hitTest(st, x, y) {
    for (const spec of st.cfg.buttons) {
        if (spec.disabled?.())
            continue; // grayed-out button ignores touches
        const c = anchorCenter(st, spec.anchor);
        if (Math.hypot(x - c.x, y - c.y) <= spec.r)
            return { kind: "button", spec };
    }
    ...
    for (const [which, spec] of sticks) {
        if (!spec)
            continue;
        const c = anchorCenter(st, spec.anchor);
        if (Math.hypot(x - c.x, y - c.y) <= spec.radius * 1.6)
            return { kind: "stick", which };
    }
    return null;
}
function updateStick(st, which, x, y) {
...
function attachListeners(st) {
    ...
    const isActuator = (e) => e.pointerType === "touch" || e.pointerType === "pen" || st.fade > 0.1;
 ...
 *      // draw(): OnscreenInput.drawControls(pad); */
export function createOnscreenGamepad(runtime, config = {}) {
    const cfg = {
    ...
    st.tracker = Object.assign(createGamepadTracker(() => {
        ...
        return fuseGamepad(touch, hw);
    }), { buttonBounds: (button) => buttonBounds(st, button) });
...
export function drawControls(pad) {
...
function paint(st) {
    ...
    for (const spec of st.cfg.buttons) {
        ...
        const c = anchorCenter(st, spec.anchor);
        const held = !disabled && [...st.pointers.values()].some((tt) => tt.kind === "button" && tt.spec === spec);
        ctx.save();
        if (disabled)
            ctx.globalAlpha *= 0.4;
        ctx.beginPath();
...
export function _resetOnscreen() {
    rectCache = null;
}
export function destroyOnscreenGamepad(pad) {
    const st = registry.get(pad);
    if (!st)
        return;
    for (const cleanup of st.cleanups.splice(0).reverse())
        cleanup();
    st.pointers.clear();
    registry.delete(pad);
}
