...
export function createWebGL2Renderer(opts = {}) {
    ...
    function uploadMesh(mesh) {
        ...
        if (!vao)
            throw new Error("WebGL2: could not create a vertex array object.");
        ...
        const attach = (location, data, size) => {
        ...
        if (!jointBuffer)
            throw new Error("WebGL2: could not create a joint buffer.");
        ...
        if (!indexBuffer)
            throw new Error("WebGL2: could not create an index buffer.");
    ...
    function uploadTexture(source, pixelated, version, repeat) {
    ...
    function setMaterial(material) {
        ...
        if (detailStrength > 0) {
            ...
            gl.bindTexture(gl.TEXTURE_2D, uploadTexture(material.detailMap, pixelated, material.detailMapVersion ?? 0, repeat));
            ...
            gl.uniform1f(u.detailOver, material.detailBlend === "over" ? (material.detailColorScale ?? 1) : 0);
            ...
            gl.uniform4f(u.detailUvTransform, detailScale[0], detailScale[1], detailOffset[0], detailOffset[1]);
            ...
            if (maskScale && (maskScale[0] !== 0 || maskScale[1] !== 0)) {
                ...
                gl.bindTexture(gl.TEXTURE_2D, uploadTexture(material.detailMask, pixelated, material.detailMaskVersion ?? 0, repeat));
                gl.uniform1i(u.detailMask, 3);
                gl.uniform4f(u.detailMaskTransform, maskScale[0], maskScale[1], maskOffset[0], maskOffset[1]);
            }
            else {
        ...
        gl.uniform4f(u.settle2, settle?.upSharpness ?? 4, settle?.baseY ?? 0, settle?.rise ?? 0, settle?.riseAmount ?? 0);
    ...
    function drawOpaque(scene, order) {
        let at = 0;
        while (at < order.length) {
            ...
            if (end - at > 1 && mesh)
                drawInstanced(scene, order, at, end, mesh, material);
            else
                for (let i = at; i < end; i++)
                    drawNode(scene.nodes[order[i]], material);
            at = end;
        }
    }
    function drawInstanced(scene, order, from, to, mesh, material) {
        const count = to - from;
        const gpu = uploadMesh(mesh);
        if (instanceData.length < count * 16)
            instanceData = new Float32Array(count * 16);
        ...
        if (!gpu.instances) {
            ...
            if (!buffer)
                throw new Error("WebGL2: could not create an instance buffer.");
        ...
        gl.drawElementsInstanced(mesh.topology === "lines" ? gl.LINES : gl.TRIANGLES, gpu.count, gpu.type, 0, count);
    ...
    const renderer = {
        ...
        render(scene, camera, opts = {}) {
            ...
            opaque.sort((a, b) => materialKey(scene.nodes[a].material) - materialKey(scene.nodes[b].material));
...
function link(gl, vertexSrc, fragmentSrc) {
...
function compile(gl, type, source) {
    ...
    if (!shader)
        throw new Error("WebGL2: could not create a shader.");
