...
export function planBlur(options, width, height, dpr) {
    const sigmaPx = options.radius * dpr;
    if (!(sigmaPx > 0) || width < 1 || height < 1)
        return null;
    let level = 0;
    while (level < MAX_LEVEL && sigmaPx / 2 ** level > MAX_SIGMA)
        level++;
...
export function subtreeOf(scene, roots) {
    const inside = new Set(roots);
    scene.nodes.forEach((n, i) => {
        if (n.parent !== undefined && inside.has(n.parent))
            inside.add(i);
    });
    return inside;
}
