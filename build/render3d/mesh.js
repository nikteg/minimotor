...
export function isSkinned(mesh) {
...
export function wireframe(mesh) {
    const idx = mesh.indices;
...
export function mergeMeshes(meshes) {
    ...
    if (lines)
        out.topology = "lines";
    if (anyNormals)
        out.normals = new Float32Array(verts * 3);
    if (anyUvs)
        out.uvs = new Float32Array(verts * 2);
    if (anyUvs1)
        out.uvs1 = new Float32Array(verts * 2);
    if (anyColors)
        out.colors = new Float32Array(verts * 4);
    let v = 0;
    let i = 0;
    for (const m of meshes) {
        ...
        if (out.normals) {
            if (m.normals)
                ...
                for (let k = 0; k < n; k++)
                    out.normals[(v + k) * 3 + 1] = 1;
        }
        if (out.uvs && m.uvs)
            out.uvs.set(m.uvs, v * 2);
        if (out.uvs1 && m.uvs1)
            out.uvs1.set(m.uvs1, v * 2);
        if (out.colors) {
            if (m.colors)
                out.colors.set(m.colors, v * 4);
            else
                out.colors.fill(1, v * 4, (v + n) * 4);
        }
...
export function box(width = 1, height = width, depth = width) {
    const x = width / 2;
    const y = height / 2;
    const z = depth / 2;
...
export function plane(width = 1, depth = width, subdivisions = 1) {
...
export function cylinder(radius = 0.5, height = 1, segments = 24, topRadius = radius, capped = true) {
    const cols = Math.max(3, Math.floor(segments));
    const y = height / 2;
    const parts = [];
    ...
    if (capped) {
        for (const [r, py, up] of [
            ...
            if (r === 0)
                continue; // a cone tip needs no cap
...
function disc(radius, segments, py, up) {
    ...
    for (let c = 0; c < segments; c++) {
        ...
        if (up === 1)
            indices.set([0, v, next], c * 3);
        else
            indices.set([0, next, v], c * 3);
    }
    return { positions, normals, uvs, indices };
}
/** A torus in the XZ plane. `radius` is to the centre of the tube. */
export function torus(radius = 0.4, tube = 0.15, segments = 32, tubeSegments = 16) {
    const cols = Math.max(3, Math.floor(segments));
