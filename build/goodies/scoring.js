...
export function timingGrade(offsetMs, windows = {}) {
    const distance = Math.abs(offsetMs);
    if (distance <= (windows.perfect ?? 35))
        return "perfect";
    if (distance <= (windows.great ?? 75))
        return "great";
    if (distance <= (windows.good ?? 130))
        return "good";
    return "miss";
}
/** Label a score from ascending thresholds, e.g. `[0,1000,5000]` and
 * `["C","B","A"]`. Scores below the first threshold use the first rank. */
export function scoreRank(score, thresholds, ranks) {
    if (ranks.length === 0)
        return undefined;
    let index = 0;
...
export function beatClock(elapsedMs, periodMs) {
