 ...
 */
export function set(image, options) {
    ...
    const factories = {
        ...
        pick(cells, weights) {
            ...
            return (at) => {
                ...
                if (!weights)
                    return cells[Math.min(cells.length - 1, Math.floor(roll))];
        ...
        auto9(base, opts = {}) {
            ...
            const cells = Array.from({ length: 9 }, (_, index) => cell(baseCol + (index % 3) * stride, baseRow + Math.floor(index / 3) * stride));
        ...
        auto16(base) {
            ...
            return (at) => {
                const mask = (at.neighbor(0, -1) ? 1 : 0) |
                    (at.neighbor(1, 0) ? 2 : 0) |
                    (at.neighbor(0, 1) ? 4 : 0) |
                    (at.neighbor(-1, 0) ? 8 : 0);
                return byMask[mask];
    ...
    const named = {};
    for (const name of Object.keys(options.names)) {
    ...
    return Object.assign(named, factories);
}
...
export function recolor(image, palette) {
    const sized = image;
