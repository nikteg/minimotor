...
import { indexRects, mesh, queryIndex } from "./mesh.js";
import { createPainter } from "./paint.js";
export function grid(source, options) {
    ...
    if (ascii) {
        ...
        cells = source.map((sourceRow) => Array.from({ length: width }, (_, cx) => sourceRow[cx] || EMPTY));
    }
    ...
    for (const [symbol, spec] of Object.entries(legend)) {
        const span = spec.span ?? [ascii ? symbol.length : 1, 1];
        spans[symbol] = span;
        const [spanCols, spanRows] = span;
        if (!Number.isInteger(spanCols) ||
    ...
    const rect = { x: 0, y: 0, w: cols * size, h: rows * size };
    function specAt(cx, cy) {
    ...
    function buildSolidIndex() {
        ...
        for (let cy = 0; cy < rows; cy++) {
            for (let cx = 0; cx < cols; cx++) {
                ...
                if (!spec.solid && !spec.slope && !exposedTop)
                    continue;
                const [spanCols, spanRows] = spans[ch] ?? ONE_CELL;
    ...
    function buildTagIndex(tag) {
        const rects = [];
        const member = new Uint8Array(cols * rows);
        for (let cy = 0; cy < rows; cy++) {
            for (let cx = 0; cx < cols; cx++) {
                const ch = cells[cy][cx];
                if (!legend[ch]?.tags?.includes(tag))
                    continue;
                const [spanCols, spanRows] = spans[ch] ?? ONE_CELL;
                if (spanCols !== 1 || spanRows !== 1) {
    ...
    const level = {
        ...
        spawnOne(char) {
            ...
            if (all.length === 0)
                throw new Error(`Tiles: no "${char}" marker in the level`);
            return all[0];
        },
        solidAt(x, y) {
            ...
            for (let cy = Math.max(0, ty - maxSpanRows + 1); cy <= ty; cy++) {
                for (let cx = Math.max(0, tx - maxSpanCols + 1); cx <= tx; cx++) {
                    ...
                    if (tx < cx + spanCols && ty < cy + spanRows)
                        return true;
                }
            }
            return false;
        ...
        render(ctx, skin, opts) {
            ...
            painter ?? (painter = createPainter(dims, cells, level));
...
export function world(maps, options) {
    const areas = Object.keys(maps);
    if (areas.length === 0)
        throw new Error("Tiles.world: add at least one level");
    ...
    const level = (area) => {
        ...
        if (!found)
            throw new Error(`Tiles.world: no level named "${area}"`);
        return found;
    };
    const endpoint = (source) => {
        const split = source.lastIndexOf(":");
        const area = source.slice(0, split);
    ...
    const connect = (fromSource, toSource, link) => {
    ...
    return {
        ...
        level,
        markers(marker) {
            return areas.flatMap((area) => level(area)
                .spawns(marker)
                .map((at) => ({ ...at, area })));
        },
        portals(area) {
