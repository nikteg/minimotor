...
export function mesh(dims, member, mergeDown, make) {
    const { cols, rows } = dims;
    const used = new Uint8Array(member.length);
    for (let cy = 0; cy < rows; cy++) {
        for (let cx = 0; cx < cols; cx++) {
            const at = cy * cols + cx;
            if (!member[at] || used[at])
                continue;
            let w = 1;
            while (cx + w < cols && member[at + w] && !used[at + w])
                w++;
            let h = 1;
...
export function indexRects(dims, rects) {
    ...
    for (const bucket of byRow)
        bucket.sort((a, b) => rects[a].x - rects[b].x);
...
export function queryIndex(dims, index, area, out) {
    const { rows, size } = dims;
    ...
    for (let row = first; row <= last; row++) {
        ...
        while (lo < hi) {
            const mid = (lo + hi) >> 1;
            if (index.rects[bucket[mid]].x < reach)
                lo = mid + 1;
            else
                hi = mid;
        }
        for (let i = lo; i < bucket.length; i++) {
            ...
            if (r.x >= right)
                break;
            if (index.seen[at] === epoch)
                continue;
            index.seen[at] = epoch;
            if (r.x + r.w > area.x && r.y < area.y + area.h && r.y + r.h > area.y)
                out.push(r);
        }
    }
    return out;
