...
import { optimizeLevels, trainPreferenceModel, } from "../../cli/features/level.js";
...
const sameConfig = (a, b) => a.ladders === b.ladders &&
    a.gems === b.gems &&
    a.dash === b.dash &&
    a.doubleJump === b.doubleJump &&
    a.wallJump === b.wallJump;
const finiteNumber = (value, max) => typeof value === "number" && Number.isFinite(value)
...
function sanitizeTelemetry(value) {
    ...
    return {
        ...
        deaths: finiteNumber(data.deaths, 10000),
        jumps: finiteNumber(data.jumps, 100000),
        dashes: finiteNumber(data.dashes, 100000),
        doubleJumps: finiteNumber(data.doubleJumps, 100000),
        wallJumps: finiteNumber(data.wallJumps, 100000),
        gems: finiteNumber(data.gems, 10000),
        maxX: finiteNumber(data.maxX, 100000),
        completed: data.completed === true,
        completionMs: finiteNumber(data.completionMs, 60 * 60 * 1000),
    };
}
export function createLevelTesterServer(ratingsPath) {
    ...
    wss.on("connection", (socket) => {
        ...
        const modelForConfig = () => {
            ...
            if (relevant.length < 3)
                return undefined;
            return trainPreferenceModel(relevant.map((row) => ({ metrics: row.metrics, rating: row.rating })));
        };
        const next = () => {
            const features = ["gaps", "platforms", "tunnels", "exit"];
            if (config.ladders)
                features.push("ladders");
            if (config.gems)
                features.push("gems");
        ...
        socket.on("message", (data) => {
            ...
            if (message.type === "configure") {
                const value = typeof message.config === "object" && message.config !== null
                    ? message.config
                    : undefined;
                config = {
