...
import { createPlatformerSimulation, TILE } from "./simulation.js";
...
function respawn(countDeath = false) {
    simulation?.reset(countDeath);
    Camera.snap();
}
function loadCandidate(next) {
    candidate = next;
...
socket.addEventListener("message", (event) => {
    ...
    if (message.type === "hello")
        ratings = message.ratings ?? ratings;
    if (message.type === "rated")
        ratings = message.total ?? ratings;
...
function rate(rating) {
    if (!candidate || !simulation || loading)
        return;
...
function reconfigure(next) {
    if (next.ladders === config.ladders &&
...
function update() {
    if (!simulation || !candidate || loading)
        return;
...
function drawLevel() {
    ...
    for (let y = 0; y < candidate.level.height; y++) {
        for (let x = 0; x < candidate.level.width; x++) {
            ...
            if (glyph === "#") {
                ...
                if (candidate.level.grid[y - 1]?.[x] !== "#")
                    Draw.rect(px, py, TILE, 3, "#64748b");
            }
            else if (glyph === "=") {
    ...
    for (const gem of gems) {
        if (!gem.taken)
            Draw.circle(gem.x, gem.y, 5, "#facc15");
    }
...
function drawHud() {
    ...
    UI.panel({ x: 12, y: 12, w: 285, title: "LEVEL TESTER", gap: 7 }, () => {
        ...
        UI.text(`heuristic ${Math.round(candidate.score.total * 100)}%  ·  predicted ${percent(candidate.predictedRating)}`, { size: 11, bold: true });
        UI.text(`trained on ${candidate.learnedFrom} matching ratings · ${ratings} total`, {
            size: 10,
            color: "dim",
        });
        UI.row({ gap: 5 }, () => {
            if (UI.button({ id: "like", label: "👍 LIKE", w: 82, h: 27 }))
                rate(1);
            if (UI.button({ id: "dislike", label: "👎 DISLIKE", w: 92, h: 27, variant: "danger" }))
                rate(0);
            if (UI.button({ id: "skip", label: "SKIP", w: 61, h: 27 }))
                skip();
        });
        const stats = simulation?.stats;
        UI.text(`${stats?.completed ? "exit reached" : "find the cyan exit"} · deaths ${stats?.deaths ?? 0} · gems ${stats?.gems ?? 0}/${simulation?.gems.length ?? 0}`, { size: 10, color: stats?.completed ? "accent" : "dim" });
    });
