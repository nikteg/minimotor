...
import { numberOption, takeFlag, takeOption } from "../../cli/utils.js";
...
function snapshot(stages, name, cells) {
...
export function generateLevel(options = {}) {
    ...
    const enabled = (feature) => features.includes(feature);
    ...
    for (const feature of features) {
        if (!generatedFeatures.includes(feature))
            throw new Error(`unknown level feature "${feature}"`);
    }
    ...
    const layoutChoices = width < 36 ? ["surface", "tunnel"] : ["surface", "tunnel", "mixed"];
    ...
    const stages = [];
    const routeFloor = Array(width).fill(height - 4);
    const ceiling = Array(width).fill(0);
    let rooms = 0;
    ...
    if (layout === "tunnel") {
        ...
        for (let x = 0; x < width; x++) {
            for (let y = routeFloor[x]; y < height; y++)
                cells[y][x] = "#";
        }
    }
    else {
        ...
        for (let x = 0; x < width; x++) {
            ...
            for (let y = elevation; y < height; y++)
                cells[y][x] = "#";
        }
    }
    ...
    if (layout === "tunnel") {
        ...
        const chamberCenters = width < 40 ? [Math.floor(width / 2)] : [0.32, 0.68].map((n) => Math.floor(width * n));
        ...
        for (let x = 0; x < width; x++) {
            for (let y = ceiling[x]; y < routeFloor[x]; y++)
                cells[y][x] = ".";
        }
    }
    else if (layout === "mixed") {
        ...
        for (let x = entry + 1; x < exit; x++)
            routeFloor[x] = deepFloor;
        ...
        for (let x = descendStart; x <= ascendEnd; x++) {
            if (x > entry && x < exit)
                ceiling[x] = Math.max(2, height - 8);
            else
                ceiling[x] = Math.max(0, routeFloor[x] - 4);
        }
        for (const center of chamberCenters) {
            if (center <= entry || center >= exit)
                continue;
            rooms++;
    ...
    let gaps = 0;
    let maxGap = 0;
    const carveGap = (x, gap) => {
        for (let gx = x; gx < Math.min(x + gap, width - 6); gx++) {
            for (let y = routeFloor[gx]; y < height; y++)
                cells[y][gx] = ".";
        }
        gaps++;
    ...
    if (enabled("gaps")) {
        for (let x = 8; x < width - 8; x += 7 + Math.floor(random() * 5)) {
            if (random() > 0.2 + difficulty * 0.55)
                continue;
            const extendedGap = abilities.dash ? (random() < 0.5 ? 3 : 4) : 3;
            const gap = (abilities.dash || abilities.doubleJump) && random() < 0.35
                ? extendedGap
                : random() < 0.75
                    ? 1
                    : 2;
        ...
        if (gaps === 0)
            carveGap(Math.floor(width / 2), difficulty > 0.75 ? 2 : 1);
    }
    ...
    if (enabled("platforms")) {
        for (let x = 7; x < width - 7; x += 10 + Math.floor(random() * 4)) {
            ...
            if (enabled("ladders")) {
                ...
                for (let wy = wallTop; wy < routeFloor[wallX]; wy++)
                    cells[wy][wallX] = "#";
            }
    ...
    if (errors.length)
        throw new Error(`generated invalid level: ${errors.join("; ")}`);
    return result;
}
...
export function validateGeneratedLevel(level) {
    ...
    if (level.grid.length !== level.height)
        errors.push("wrong row count");
    if (level.grid.some((row) => row.length !== level.width))
        errors.push("wrong column count");
    if (level.grid.join("").split("P").length - 1 !== 1)
        errors.push("expected one player");
    const enabled = (feature) => level.features.includes(feature);
    ...
    if (level.metrics.maxStep > 1)
        errors.push("terrain step exceeds 1 tile");
...
export function generatedDesign(level, id = "Generated") {
    const entities = [];
    const visited = new Set();
    const at = (x, y) => level.grid[y]?.[x] ?? ".";
    for (let y = 0; y < level.height; y++) {
        for (let x = 0; x < level.width; x++) {
            ...
            if (glyph === "#" || glyph === "=") {
                let width = 1;
                while (x + width < level.width && at(x + width, y) === glyph)
                    width++;
                for (let offset = 0; offset < width; offset++)
                    visited.add(`${x + offset},${y}`);
                entities.push({ type: glyph === "#" ? "Solid" : "OneWay", x, y, w: width, h: 1 });
            }
            else if (glyph === "H") {
                let height = 1;
                while (y + height < level.height && at(x, y + height) === "H")
                    height++;
...
export function scoreGeneratedLevel(level, profile = "balanced") {
    const columns = Array.from({ length: level.width }, (_, x) => level.grid.findIndex((row) => row[x] === "#"));
    const grounded = columns.filter((height) => height >= 0);
    const events = [];
    for (let x = 0; x < level.width; x++) {
        if (level.grid.some((row) => "=HGE".includes(row[x])))
            events.push(x);
    }
    const spacings = events.slice(1).map((x, index) => Math.min(8, x - events[index]));
    const signatures = new Set(Array.from({ length: level.width }, (_, x) => level.grid.map((row) => row[x]).join("")));
    const metrics = {
    ...
    const components = {
        ...
        composition: targetScore(metrics.enclosureRatio, level.layout === "surface" ? 0 : level.layout === "tunnel" ? 0.92 : 0.55, level.layout === "surface" ? 0.2 : 0.5) * (level.layout === "surface" ? 1 : clamp01(metrics.roomCount / 2)),
    };
    const weights = {
        validity: 3,
        leniency: 1.2,
    ...
    const weighted = Object.entries(weights).reduce((sum, [name, weight]) => sum + components[name] * weight, 0);
    return {
...
function solveLinearSystem(matrix, values) {
    ...
    return augmented.map((row, index) => Number.isFinite(row[size]) ? row[size] : index === 0 ? 0.5 : 0);
}
/** Fit a small ridge-regression preference model from human or agent ratings. */
export function trainPreferenceModel(rows, ridge = 0.1) {
    const labeled = rows.filter((row) => typeof row.rating === "number" && Number.isFinite(row.rating));
    if (labeled.length < 3)
        throw new Error("training needs at least 3 labeled rows");
    const vectors = labeled.map((row) => preferenceFeatures.map((name) => row.metrics[name]));
    const means = preferenceFeatures.map((_, column) => vectors.reduce((sum, vector) => sum + vector[column], 0) / vectors.length);
    const scales = preferenceFeatures.map((_, column) => {
        const variance = vectors.reduce((sum, vector) => sum + (vector[column] - means[column]) ** 2, 0) /
            vectors.length;
    ...
    for (let row = 0; row < inputs.length; row++) {
        for (let x = 0; x < size; x++) {
            ...
            for (let y = 0; y < size; y++)
                matrix[x][y] += inputs[row][x] * inputs[row][y];
        }
    }
...
export function predictPreference(model, metrics) {
    const standardized = preferenceFeatures.map((name, index) => (metrics[name] - model.means[index]) / model.scales[index]);
    return clamp01(model.weights[0] +
...
function takeGeneratedFeatures(args) {
    const featureList = takeOption(args, "--features");
    const without = repeatedOptions(args, "--without");
    if (featureList && without.length)
        throw new Error("--features and --without cannot be combined");
    const requested = (featureList ? featureList.split(",") : generatedFeatures).filter((feature) => !without.includes(feature));
    for (const feature of requested) {
        if (!generatedFeatures.includes(feature)) {
            throw new Error(`unknown level feature "${feature}" (choose from ${generatedFeatures.join(", ")})`);
        }
    }
    return requested;
}
function takeGeneratedAbilities(args) {
    return {
...
function takeGeneratedLayout(args) {
    const layout = takeOption(args, "--layout") ?? "varied";
    if (layout === "varied")
        return undefined;
    if (layout === "surface" || layout === "tunnel" || layout === "mixed")
        return layout;
    throw new Error('--layout must be "varied", "surface", "tunnel", or "mixed"');
}
function takeScoreProfile(args) {
...
export function checkLevelProject(project, options = {}) {
    const errors = [];
    const warnings = [];
    const report = [];
    ...
    const definitions = new Map((project.defs?.entities ?? []).map((definition) => [
    ...
    for (const level of project.levels ?? []) {
        ...
        const tags = (entity) => definitions.get(entity.__identifier) ?? [];
        const has = (entity, tag) => tags(entity).includes(tag);
        const supports = new Set();
        const ladders = new Set();
        const key = (x, y) => `${x},${y}`;
        const cell = (value) => value / grid;
        const countTag = (tag) => entities.filter((entity) => has(entity, tag)).length;
        for (const entity of entities) {
            const entityTags = tags(entity);
            const gameplay = entityTags.some((tag) => tag.startsWith("mm:")) && !entityTags.includes("mm:sprite");
            if (gameplay &&
                [entity.px[0], entity.px[1], entity.width, entity.height].some((value) => value % grid !== 0)) {
            ...
            if (has(entity, "mm:solid") || has(entity, "mm:one-way")) {
                for (let cy = y; cy < y + h; cy++) {
                    for (let cx = x; cx < x + w; cx++)
                        supports.add(key(cx, cy));
                }
            }
            if (has(entity, "mm:one-way") && w < 3) {
                errors.push(`${level.identifier}/${entity.__identifier}@${x},${y}: platform is under 3 tiles`);
            }
            if (has(entity, "mm:ladder")) {
                if (h < 3) {
                    errors.push(`${level.identifier}/${entity.__identifier}@${x},${y}: ladder is under 3 tiles`);
                }
                for (let cy = y; cy < y + h; cy++)
                    ladders.add(key(x, cy));
            }
            if (has(entity, "mm:portal") && entity.iid) {
                ...
                portalIds.set(entity.iid, {
                    ...
                    target: reference && typeof reference.entityIid === "string" ? reference.entityIid : undefined,
                });
        ...
        for (const entity of entities.filter((candidate) => has(candidate, "mm:ladder"))) {
            ...
            if (!supports.has(key(x, y - 1))) {
                errors.push(`${level.identifier}/${entity.__identifier}@${x},${y}: no readable top landing`);
            }
        }
        if (art) {
            const tileset = art.__tilesetDefUid === null || art.__tilesetDefUid === undefined
                ? undefined
                : tilesets.get(art.__tilesetDefUid);
            if (!tileset)
                errors.push(`${level.identifier}/Art: missing tileset definition`);
            for (const tile of [...(art.autoLayerTiles ?? []), ...(art.gridTiles ?? [])]) {
                if (tile.px[0] % art.__gridSize !== 0 ||
        ...
        const targets = entities.filter((candidate) => targetTypes.has(candidate.__identifier) || has(candidate, "mm:portal"));
        for (const entity of targets) {
            targetCount++;
            const x = cell(entity.px[0]);
            const y = cell(entity.px[1]) + (has(entity, "mm:portal") ? cell(entity.height) - 1 : 0);
            if (!reached.has(key(x, y))) {
                errors.push(`${level.identifier}/${entity.__identifier}@${x},${cell(entity.px[1])}: unreachable`);
            }
        }
...
function ascii(level, trace) {
    const render = (name, grid) => `# ${name}\n${grid.join("\n")}`;
    if (trace) {
        return `${level.stages.map((stage) => render(stage.name, stage.grid)).join("\n\n")}\n`;
    }
    return `${render(`${level.seed} · ${level.layout} · rooms ${level.metrics.rooms} · gaps ${level.metrics.gaps} · platforms ${level.metrics.platforms}`, level.grid)}\n`;
}
export default defineFeature({
    ...
    async run(input) {
        ...
        if (command === "test") {
            ...
            const ratingsPath = resolve(takeOption(args, "--ratings") ?? ".minimotor/level-tester-ratings.jsonl");
            if (args.length)
                throw new Error(`unknown option "${args[0]}"`);
            if (!Number.isInteger(port) || port < 1 || port > 65535) {
                throw new Error("--port must be an integer from 1 to 65535");
            }
            const { startStandaloneLevelTester } = await import("../../cli/level-tester/standalone.js");
            const tester = await startStandaloneLevelTester({ host, port, ratingsPath });
            process.stdout.write(`Level tester: ${tester.url}\nRatings: ${ratingsPath}\nPress Ctrl+C to stop.\n`);
        ...
        if (command === "simulate") {
            ...
            const maxSteps = numberOption(args, 1500, "--max-steps");
            ...
            for (const [name, value, maximum] of [
                ...
                ["--max-steps", maxSteps, 20000],
            ]) {
                if (!Number.isInteger(value) || value < 1 || value > maximum) {
                    throw new Error(`${name} must be an integer from 1 to ${maximum}`);
                }
            }
            if (levels * rounds > 1000) {
                throw new Error("--levels × --rounds cannot exceed 1000 candidates");
            }
            if (levels * rounds * bots * attempts > 200000) {
                throw new Error("candidate × bot × attempt count cannot exceed 200000 episodes");
            }
            if (!features.includes("exit"))
                throw new Error("bot simulation requires the exit feature");
            const { evaluateLevelWithBots } = await import("../../cli/level-tester/bots.js");
            const evaluated = [];
            const roundReports = [];
            ...
            for (let round = 0; round < rounds; round++) {
                ...
                const successRate = roundCandidates.reduce((sum, candidate) => sum + candidate.bot.metrics.successRate, 0) /
                    roundCandidates.length;
                roundReports.push({ round, difficulty: adaptiveDifficulty, successRate });
                if (successRate > 0.75)
                    adaptiveDifficulty = Math.min(1, adaptiveDifficulty + 0.1);
                else if (successRate < 0.35) {
                    adaptiveDifficulty = Math.max(0, adaptiveDifficulty - 0.1);
                }
            }
            evaluated.sort((a, b) => Number(b.bot.passed) - Number(a.bot.passed) || b.fitness - a.fitness);
            ...
            if (output) {
                if (!best)
                    throw new Error("no generated candidate passed bot evaluation");
        ...
        if (command === "evolve") {
            ...
            const maxSteps = numberOption(args, 1800, "--max-steps");
            const width = numberOption(args, 48, "--width");
            const height = numberOption(args, 22, "--height");
            const difficulty = numberOption(args, 0.45, "--difficulty");
            const profile = takeScoreProfile(args);
            ...
            if (!features.includes("exit"))
                throw new Error("evolution requires the exit feature");
            const { evolveLevels } = await import("../../cli/level-tester/tournament.js");
            ...
            if (treePath)
                writeFileSync(treePath, result.tree);
            if (reportPath)
                writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`);
            if (archivePath) {
                ...
                const distinct = [...result.candidates]
                    .sort((left, right) => Number(right.bot.passed) - Number(left.bot.passed) || right.fitness - left.fitness)
                    .filter((candidate) => {
                    if (seen.has(candidate.seed))
                        return false;
                    seen.add(candidate.seed);
                    return true;
                });
                const layoutQuota = Math.floor(keep / 3);
                const retained = ["surface", "tunnel", "mixed"].flatMap((layout) => distinct.filter((candidate) => candidate.level.layout === layout).slice(0, layoutQuota));
                const retainedSeeds = new Set(retained.map((candidate) => candidate.seed));
                retained.push(...distinct
                    .filter((candidate) => !retainedSeeds.has(candidate.seed))
                    .slice(0, keep - retained.length));
                retained.sort((left, right) => Number(right.bot.passed) - Number(left.bot.passed) || right.fitness - left.fitness);
                retained.forEach((candidate, index) => {
                    const name = `${String(index + 1).padStart(3, "0")}-${candidate.id}`;
                    writeFileSync(resolve(archivePath, `${name}.json`), `${JSON.stringify(generatedDesign(candidate.level, candidate.id), null, 2)}\n`);
                    writeFileSync(resolve(archivePath, `${name}.txt`), ascii(candidate.level, false));
                });
                writeFileSync(resolve(archivePath, "manifest.json"), `${JSON.stringify(retained.map((candidate, index) => ({
                    rank: index + 1,
                    id: candidate.id,
                    parentId: candidate.parentId,
                    seed: candidate.seed,
        ...
        if (command === "train") {
            ...
            if (!output)
                throw new Error("mm level train needs --out");
            if (args.length)
                throw new Error(`unknown option "${args[0]}"`);
        ...
        if (command === "score") {
            ...
            process.stdout.write(`${JSON.stringify({
                ...
                ...(modelPath
                    ? {
                        predictedRating: predictPreference(JSON.parse(readFileSync(modelPath, "utf8")), score.metrics),
                    }
                    : {}),
            }, null, 2)}\n`);
            return;
        ...
        if (command === "optimize") {
            ...
            process.stdout.write([
                ...
                ...shown.map((candidate, index) => `${index + 1}. ${candidate.seed} [${candidate.level.layout}] fitness=${candidate.fitness.toFixed(4)} heuristic=${candidate.score.total.toFixed(4)} difficulty=${candidate.difficulty.toFixed(3)} vertical=${candidate.score.metrics.verticalRange} density=${candidate.score.metrics.eventDensity.toFixed(3)}`),
            ].join("\n") + "\n");
            if (output) {
                writeFileSync(output, `${JSON.stringify(generatedDesign(result.best.level), null, 2)}\n`);
            }
            if (dataset) {
                const rows = result.evaluated.map((candidate) => JSON.stringify({
                    seed: candidate.seed,
                    difficulty: candidate.difficulty,
                    layout: candidate.level.layout,
                    features: candidate.level.features,
        ...
        if (command === "build") {
            ...
            if (args.length)
                throw new Error(`unknown option "${args[0]}"`);
            const load = async (path) => (await import(pathToFileURL(resolve(path)).href));
            const design = designPath.endsWith(".json")
                ? JSON.parse(readFileSync(designPath, "utf8"))
                : await load(designPath).then((module) => module.design ?? module.default);
            if (!design)
                throw new Error(`${designPath} must export design or a default design`);
        ...
        if (command === "check") {
            ...
            if (!path || path.startsWith("-"))
                throw new Error("mm level check needs an LDtk project");
            ...
            if (verbose) {
                process.stdout.write(`Movement envelope: ${jumpX} across, ${jumpUp} up, ${fall} down (tiles)\n${result.report.join("\n")}\n`);
            }
            process.stdout.write(`valid: ${path} (${result.levels} levels, ${result.targets} reachable targets)\n`);
            return;
        }
        if (command !== "generate") {
            throw new Error("usage: mm level <test|simulate|evolve|generate|score|optimize|train|build|check> [options]");
        }
