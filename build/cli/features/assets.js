...
import { files, numberOption, takeFlag } from "../../cli/utils.js";
...
const validateAseprite = (data) => {
    ...
    const errors = [];
    if (frames.length === 0)
        errors.push("Aseprite atlas has no frames");
    frames.forEach((value, index) => {
        const frame = value;
        const geometry = frame.frame;
        if (!geometry ||
            ![geometry.x, geometry.y, geometry.w, geometry.h, frame.duration].every((part) => typeof part === "number" && Number.isFinite(part)) ||
            geometry.w <= 0 ||
            geometry.h <= 0 ||
            frame.duration <= 0) {
...
export function checkAssets(root, tileSize) {
    ...
    const issues = [];
    const names = new Map();
    const report = (level, path, message) => issues.push({ level, file: relative(root, path), message });
    for (const path of paths) {
        ...
        try {
            if (extension === ".json" || extension === ".ldtk" || extension === ".ldtkl") {
                ...
                for (const message of validateAseprite(data))
                ...
                if (!size)
                    report("error", path, "invalid PNG header");
    ...
    for (const path of paths) {
        ...
        for (const match of source.matchAll(reference)) {
            ...
            if (!existsSync(target))
                report("error", path, `missing referenced asset ${match[1]}`);
        }
    }
...
export default defineFeature({
    ...
    run(input) {
        ...
        if (json)
            ...
            process.stdout.write(`${issues.length ? "checked" : "valid"}: ${basename(directory)} (${issues.length} issues)\n`);
        }
        const failed = issues.some((issue) => issue.level === "error" || (strict && issue.level === "warning"));
        if (failed)
