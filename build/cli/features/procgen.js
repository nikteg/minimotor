...
import { analyze, caves, dungeon, fromText, isConnected, DUNGEON_MARKERS, measure, repair, rooms, steer, synthesize, toText, } from "../../procgen/index.js";
...
`;
const KINDS = ["dungeon", "cave", "rooms", "wfc"];
...
export function generate(options) {
    ...
    switch (options.kind) {
        ...
        case "wfc": {
            if (!options.sample)
                throw new Error("wfc needs --sample <file>");
...
function report(grid) {
    ...
    return {
        ...
        frequencies: Object.fromEntries(Object.entries(stats.frequencies).map(([glyph, share]) => [glyph, Number(share.toFixed(3))])),
    };
}
export default defineFeature({
    ...
    run(input) {
        ...
        if (command === "measure") {
            ...
            if (!path)
                throw new Error("measure needs a level file");
            if (args.length)
        ...
        if (command !== "gen")
            throw new Error(`unknown procgen command "${command}"\n\n${help}`);
        ...
        if (check) {
            if (!out)
                throw new Error("--check needs --out <file> to compare against");
        ...
        if (json)
            process.stdout.write(`${JSON.stringify(report(fromText(text)), null, 2)}\n`);
    },
});
