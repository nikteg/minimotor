...
async function features() {
    ...
    return Promise.all(files.map(async (file) => {
        const module = (await import(pathToFileURL(resolve(featureDirectory, file)).href));
        if (!module.default)
            throw new Error(`${file} does not export a default feature`);
        return module.default;
    }));
}
function help(available) {
...
export async function main(args = process.argv.slice(2)) {
    ...
    if (!feature)
        throw new Error(`unknown feature "${args[0]}"\n\n${help(available)}`);
