...
function premultiplied(image, x, y, out) {
...
export function sliceIds(image, rect, axis) {
...
export const idStrip = (ids) => ids.map((id) => (id < ALPHABET.length ? ALPHABET[id] : "#")).join("");
...
export function bandPeriod(ids, from, to) {
    const length = to - from;
    if (length <= 0)
        return 0;
    for (let period = 1; period <= length >> 1; period++) {
        let matches = true;
        for (let i = from + period; i < to && matches; i++)
            matches = ids[i] === ids[i - period];
        if (matches)
            return period;
    }
    return length;
...
export function longestPeriodicRun(ids) {
    const n = ids.length;
    const middle = n >> 1;
    let best;
...
function sliceDistance(image, rect, axis, a, b) {
    const depth = axis === "x" ? rect.sh : rect.sw;
    const left = [0, 0, 0, 0];
    const right = [0, 0, 0, 0];
    let total = 0;
...
function analyzeAxis(image, rect, axis, lead, trail) {
...
const alphaAt = (image, x, y) => x < 0 || y < 0 || x >= image.width || y >= image.height
    ? 0
...
export function contentBounds(image, rect) {
    let minX = rect.sw;
    let minY = rect.sh;
    let maxX = -1;
    let maxY = -1;
    for (let y = 0; y < rect.sh; y++) {
        for (let x = 0; x < rect.sw; x++) {
            if (alphaAt(image, rect.sx + x, rect.sy + y) === 0)
                continue;
            if (x < minX)
                minX = x;
            if (y < minY)
...
function touchesNeighbour(image, rect) {
    for (let x = rect.sx - 1; x <= rect.sx + rect.sw; x++) {
        if (alphaAt(image, x, rect.sy - 1) > 0)
            return true;
        if (alphaAt(image, x, rect.sy + rect.sh) > 0)
            return true;
    }
    for (let y = rect.sy; y < rect.sy + rect.sh; y++) {
        if (alphaAt(image, rect.sx - 1, y) > 0)
            return true;
        if (alphaAt(image, rect.sx + rect.sw, y) > 0)
            return true;
    }
    return false;
}
...
function axisFindings(name, report, size) {
    ...
    if (report.center <= 0) {
        findings.push({
            ...
            region: name,
            code: "no-center",
            message: `${leadName}+${trailName} (${report.lead}+${report.trail}) leaves no centre band in a ` +
    ...
    if (report.periodic && report.center % report.period !== 0) {
        ...
        findings.push({
            ...
            region: name,
            code: "phase-break",
            message: `centre band is ${report.center}px but its art repeats every ${report.period}px. ` +
    ...
    if (report.derived) {
        ...
        if (generous.length) {
            findings.push({
                ...
                region: name,
                code: "inset-generous",
                message: `the frame's repeating unit runs ${runStart}–${runEnd} and ${generous.join(", ")}. ` +
        ...
        const surplus = report.keys.slice(report.lead, Math.min(runStart, bandEnd)) +
        ...
        if ((report.lead < runStart || bandEnd > runEnd) && runEnd - runStart >= surplus.length) {
            ...
            const widen = [];
            if (report.lead !== runStart)
                widen.push(`${leadName}: ${report.lead} → ${runStart}`);
            const newTrail = size - runStart - whole;
            if (report.trail !== newTrail)
                widen.push(`${trailName}: ${report.trail} → ${newTrail}`);
            findings.push({
                level: foreign ? "warning" : "info",
                region: name,
                code: "centre-too-wide",
                message: `centre band spans ${report.lead}–${bandEnd} but the art only repeats over ` +
...
export function deriveInsets(image, rect) {
    ...
    if (!outside) {
        ...
        if (!content) {
            ...
            findings.push({
                ...
                message: `the rect includes ${content.sx}px left / ${content.sy}px top / ` +
            ...
            findings.push({
                ...
                region: name,
                code: "no-padding",
                message: "art fills the rect edge-to-edge and the atlas has opaque pixels right outside it — " +
...
export function overlaps(regions) {
    const findings = [];
    for (let i = 0; i < regions.length; i++) {
        for (let j = i + 1; j < regions.length; j++) {
            const a = regions[i].rect;
            const b = regions[j].rect;
            const hit = a.sx < b.sx + b.sw && b.sx < a.sx + a.sw && a.sy < b.sy + b.sh && b.sy < a.sy + a.sh;
            if (!hit)
                continue;
...
export function compose(image, rect, insets, w, h) {
    ...
    const tile = (sx, sy, sw, sh, dx, dy, dw, dh) => {
        ...
        for (let y = 0; y < dh; y += sh) {
            for (let x = 0; x < dw; x += sw) {
                blit(sx, sy, Math.min(sw, dw - x), Math.min(sh, dh - y), dx + x, dy + y, Math.min(sw, dw - x), Math.min(sh, dh - y));
            }
        }
    };
...
export function phaseBreaks(image, rect, insets, size) {
    ...
    const scan = (axis) => {
        ...
        const period = bandPeriod(sliceIds(image, rect, axis), lead, (axis === "x" ? rect.sw : rect.sh) - trail);
        const breaks = [];
        if (period <= 0)
            return breaks;
