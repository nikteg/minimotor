...
function elide(strip, lead, trail, period) {
...
function formatAxis(report, size) {
    const names = report.axis === "x" ? ["left", "right"] : ["top", "bottom"];
    const head = `  ${report.axis}  span=${size} ${names[0]}=${report.lead} ${names[1]}=${report.trail} ` +
...
export const rank = (findings) => [...findings].sort((a, b) => LEVEL_ORDER[a.level] - LEVEL_ORDER[b.level]);
...
export function pixelMap(image, rect, insets) {
    if (rect.sw > 100 || rect.sh > 48)
        return undefined;
    const rows = [];
    const bar = (fill) => fill.repeat(insets.left) +
        "+" +
        fill.repeat(Math.max(0, rect.sw - insets.left - insets.right)) +
        "+" +
        fill.repeat(insets.right);
    for (let y = 0; y < rect.sh; y++) {
        if (y === insets.top || y === rect.sh - insets.bottom)
            rows.push(bar("-"));
        let row = "";
        for (let x = 0; x < rect.sw; x++) {
            if (x === insets.left || x === rect.sw - insets.right)
                row += "|";
            ...
            if (alpha === 0)
                ...
                const luma = (image.data[at] * 0.299 + image.data[at + 1] * 0.587 + image.data[at + 2] * 0.114) / 255;
                row +=
...
function stamp(target, source, ox, oy, zoom) {
...
export function annotate(image, rect, insets, zoom = 8) {
    const target = {
        width: rect.sw * zoom,
        height: rect.sh * zoom,
...
export function previewSheet(image, rect, insets, zoom = 4) {
