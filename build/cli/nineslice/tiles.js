...
import { analyzeRegion } from "./analyze.js";
/** Rect of one tile in a grid, by column and row. */
export const cellRect = (grid, column, row) => ({
...
const strip = (image, rect, edge) => {
    ...
    for (let i = 0; i < length; i++) {
        ...
        parts.push(alpha === 0 ? "_" : `${image.data[at]}.${image.data[at + 1]}.${image.data[at + 2]}.${alpha}`);
    }
    return parts.join(",");
};
/** Read a tile's four edge sockets. */
export const sockets = (image, rect) => ({
...
function assemble(image, cells, tile) {
    const width = tile.w * 3;
    const out = { width, height: tile.h * 3, data: new Uint8Array(width * tile.h * 3 * 4) };
...
export function analyzeTileFrame(image, grid, cells, name = "frame") {
    const findings = [];
    ...
    const outside = all.some((rect) => rect.sx < 0 ||
        rect.sy < 0 ||
    ...
    if (top.west !== bottom.west || top.east !== bottom.east) {
        findings.push({
            ...
            region: name,
            code: "asymmetric",
            message: "the top and bottom edge tiles present different side sockets, so the frame is not " +
 ...
 *  happens to store it. */
export function analyzeAutotile(image, grid, tiles, name = "autotile") {
    const findings = [];
    ...
    for (const [state, entries] of byState) {
        ...
        if (distinct.size <= 1)
            continue;
        const groups = [...distinct].map((socket) => `[${entries
        ...
        findings.push({
            ...
            message: `${entries.length} tiles declare ${state} but present ${distinct.size} different edges: ` +
    ...
    for (let mask = 0; mask < 16; mask++) {
        if (!masks.has(mask)) {
            findings.push({
                ...
                region: name,
                code: "missing-mask",
                message: `no tile answers neighbour mask ${mask} (${SIDES.filter(([, bit]) => mask & bit)
                    .map(([side]) => side)
                    .join("+") || "isolated"}) — that placement will fall back to whatever the engine defaults to`,
            });
        }
    }
    return findings;
