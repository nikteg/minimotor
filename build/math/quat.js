...
export const Quat = {
    ...
    fromAxisAngle(q, ax, ay, az, angle) {
    ...
    equals(a, b, epsilon = 1e-6) {
        const same = Math.abs(a.x - b.x) <= epsilon &&
            Math.abs(a.y - b.y) <= epsilon &&
            Math.abs(a.z - b.z) <= epsilon &&
            Math.abs(a.w - b.w) <= epsilon;
        const flipped = Math.abs(a.x + b.x) <= epsilon &&
            Math.abs(a.y + b.y) <= epsilon &&
            Math.abs(a.z + b.z) <= epsilon &&
            Math.abs(a.w + b.w) <= epsilon;
        return same || flipped;
