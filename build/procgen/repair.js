...
import { floodFill } from "../goodies/grid.js";
import { at, cloneGrid, cols, rows } from "./grid.js";
/** Find every connected walkable region, largest first. */
export function regions(grid, options = {}) {
    const width = cols(grid);
    const height = rows(grid);
    const walkable = new Set([options.floor ?? ".", ...(options.alsoWalkable ?? [])]);
    const seen = new Uint8Array(width * height);
    const found = [];
    const passable = (x, y) => x >= 0 && y >= 0 && x < width && y < height && walkable.has(grid[y][x]);
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            if (seen[y * width + x] || !passable(x, y))
                continue;
    ...
    found.sort((a, b) => b.cells.length - a.cells.length || a.bounds.y - b.bounds.y || a.bounds.x - b.bounds.x);
    return found;
}
...
export function repair(grid, options = {}) {
    ...
    for (const cell of trunk.cells)
        inTrunk[cell.y * width + cell.x] = 1;
    ...
    while (found.length > 0) {
        ...
        for (const cell of found[bestRegion].cells)
            inTrunk[cell.y * width + cell.x] = 1;
...
export function isConnected(grid, options = {}) {
...
export function sealEdges(grid, wall = "#") {
    const out = cloneGrid(grid);
    const width = cols(out);
    const height = rows(out);
    for (let x = 0; x < width; x++) {
...
export function openNeighbours(grid, x, y, floor = ".") {
    let open = 0;
    if (at(grid, x, y - 1, "#") === floor)
        open++;
    if (at(grid, x + 1, y, "#") === floor)
        open++;
    if (at(grid, x, y + 1, "#") === floor)
        open++;
    if (at(grid, x - 1, y, "#") === floor)
