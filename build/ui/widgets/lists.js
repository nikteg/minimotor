...
export function wheelScroll(p, area, offset, max) {
    const delta = claimWheel(pointInRect(p.x, p.y, area), p.wheel, offset <= 0.5, offset >= max - 0.5);
...
export function scrollGestureActive() {
    const bs = bodyScrollSlot();
...
export function dismissedByOutsideRelease(p, ...rects) {
    if (!p.released || scrollGestureActive() || pointerGestureOwned())
        return false;
...
const focusedRows = sweptCache();
function focusedRowIndex(key, focused, rowId, count) {
...
export function dragScroll(key, rect, axis, offset, max) {
    ...
    if (fling !== undefined && !bs.drag) {
        if (p.pressed && pointInRect(p.x, p.y, rect)) {
            ...
            if (next <= 0 || next >= max || Math.abs(decayed) < FLING_STOP)
                bs.momentum.delete(key);
            else
                bs.momentum.set(key, decayed);
        }
    }
    ...
    if (drag?.id === key) {
        if (p.down) {
            ...
            if (!drag.active && Math.abs(d) > DRAG_THRESHOLD)
                drag.active = true;
            if (drag.active) {
                const frames = elapsedFrames();
            ...
            if (drag.active) {
                ...
                if (Math.abs(drag.vel) > FLING_MIN)
                    bs.momentum.set(key, drag.vel);
            }
            bs.drag = null;
        }
    }
...
export function list(opts, row) {
    ...
    if (opts.rowId) {
        ...
        if (focusedIndex >= 0) {
            ...
            if (top < offset)
                offset = top;
            else if (top + heights[focusedIndex] > offset + h)
                offset = top + heights[focusedIndex] - h;
    ...
    clip({ x, y, w: listW, h }, () => {
        ...
        let last = first;
        while (last < opts.count && tops[last] < offset + h)
            last++;
...
export function grid(opts, cell) {
...
export function scrollbarFade(id, hovered, overflows) {
    const FAINT = 0.28;
...
export function scrollbar(opts) {
    const ctx = uiCtx();
    ensureWired();
    ...
    let offset = clamp(opts.offset, 0, max);
    if (max <= 0)
        return 0; // everything fits — draw nothing
    ...
    if (opts.wheelArea) {
        offset += claimWheel(pointInRect(p.x, p.y, opts.wheelArea), p.wheel, offset <= 0.5, offset >= max - 0.5);
    }
    offset = clamp(offset, 0, max);
