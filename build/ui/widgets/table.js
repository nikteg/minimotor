...
import { fillRect, resolveThemePadding, text, textWidth, theme, uiPointer, } from "../../ui/core/index.js";
...
export function table(opts) {
    ...
    const rowToken = (i) => (opts.rowKey ? opts.rowKey(rows[i]) : String(i));
    const rowWidgetId = (i) => `${opts.id}:r:${rowToken(i)}`;
    ...
    const offset = list({
        ...
        const p = uiPointer();
        const overWidget = widgetSpans.some((span) => pointInRect(p.x, p.y, { x: span.x, y: rowRect.y, w: span.w, h: rowHeight }));
