...
import { centeredText, drawBox, resolveThemePadding, roundRectPath, theme, uiFont, } from "../../ui/core/index.js";
const ZERO_INSETS = { left: 0, top: 0, right: 0, bottom: 0 };
function panelFrameInsets() {
...
export function panelTitleBodyOffset() {
...
export function paintFrame(ctx, opts) {
    ctx.save();
    ...
    if (opts.title) {
        ...
        if (!titleFrame) {
            ...
            ctx.save();
            roundRectPath(ctx, opts.x + bw, opts.y + bw, Math.max(0, opts.w - bw * 2), Math.max(0, opts.h - bw * 2), Math.max(0, theme.radius - bw));
            ctx.clip();
        ...
        const titleInsetX = Math.max(bodyPad.left - panelInsets.left, titleFrame?.insets.left ?? 0) + titleTextPad.left;
        centeredText(ctx, opts.title, titleRect.x + titleInsetX, titleRect.y + titleH / 2 + titleTextPad.top, Math.max(1, titleRect.w - titleInsetX * 2));
    }
    if (opts.highlight) {
        ...
        ctx.lineWidth = width;
        roundRectPath(ctx, opts.x + half, opts.y + half, Math.max(0, opts.w - width), Math.max(0, opts.h - width), Math.max(0, theme.radius - half));
        ctx.stroke();
    }
    ctx.restore();
}
