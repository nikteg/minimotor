...
import { navigation } from "../../input/gamepad.js";
...
const focusOf = (app) => withUiApp(app, fs);
...
export function hasActiveNavPad() {
    const s = fs();
...
export function setWidgetFocus(id) {
    const s = fs();
    if (s.focused === id)
        return;
...
export function focusReveal(seen) {
    const s = fs();
    if (!s.visible || s.revealEpoch === seen || !s.focused)
        return null;
    // This frame's registry first (the widget may have just moved), else the
    // completed one from last frame.
    const entry = s.frame.find((e) => e.id === s.focused) ?? s.registry.find((e) => e.id === s.focused);
    return entry?.rect ? { epoch: s.revealEpoch, rect: entry.rect } : null;
}
export function moveWidgetFocus(direction) {
    const s = fs();
    const entries = focusCandidates();
    if (!entries.length)
        return setWidgetFocus(null);
...
export function wireFocusCanvas(ctx, app) {
    const canvas = ctx.canvas;
    if (focusCanvases.has(canvas))
        return;
...
export function markFocusableOverlay(id) {
...
export function registerFocusProxy(id, rect) {
...
export function focusProxies(id) {
...
export function focusProxyHovered(id) {
    const proxies = fs().proxies;
    if (proxies.length === 0)
        return false;
    const p = uiPointer();
...
export function focusFromPointer(ctx, id) {
    if (!id)
        return;
    const s = fs();
    s.visible = false;
...
export function drawFocusRing(ctx, rect) {
    ...
    if (cursor) {
        ...
        ctx.drawImage(image, region.sx, region.sy, region.sw, region.sh, rect.x - region.sw - 4, rect.y + (rect.h - region.sh) / 2, region.sw, region.sh);
...
export function consumeKeyboardCommand(id) {
    const s = fs();
    if (!id || s.command?.id !== id)
        return null;
...
function repeatPulse(state, key) {
    if (!key) {
        state.key = null;
...
export function padNav() {
    ...
    for (const pad of pads) {
        ...
        if (Math.abs(nav.x) > Math.abs(x))
            x = nav.x;
        if (Math.abs(nav.y) > Math.abs(y))
            y = nav.y;
        accept || (accept = nav.acceptPressed);
        cancel || (cancel = nav.cancelPressed);
    }
    const T = 0.5;
    const vertical = y > T ? "ArrowDown" : y < -T ? "ArrowUp" : null;
    if (repeatPulse(s.repeatV, vertical)) {
        s.visible = true;
        const dir = vertical === "ArrowDown" ? 1 : -1;
    ...
    if (horizontalPulse)
        s.command = { id: s.focused, key: horizontal };
}
...
export function wireFocusKeyboard() {
    ...
    window.addEventListener("keydown", (event) => {
        ...
        withUiApp(app, () => {
            ...
            if (event.key === "Tab")
                s.visible = true;
            const onFocusSurface = !!s.focused ||
    ...
    window.addEventListener("focusin", (event) => {
        const target = event.target;
        if (target?.dataset?.minimotorUi !== "true" &&
            !(target instanceof HTMLCanvasElement && focusCanvases.has(target))) {
...
// next frame.
export function focusEndFrame() {
    const s = fs();
    s.registry = s.frame;
    s.frame = [];
    s.proxies.length = 0;
    const wasFocusOverlay = s.overlayActive;
    if (!wasFocusOverlay && s.trapSeen)
        s.beforeOverlay = s.focused;
    ...
    if (wasFocusOverlay && !s.overlayActive)
        s.beforeOverlay = null;
    s.trapSeen = false;
    s.trapFocusVisible = false;
    s.dismissRequested = false;
}
/** An overlay ran this frame — trap focus into it (called by `enterOverlay`). */
export function markFocusTrap(focusVisible = false) {
    const s = fs();
    s.trapSeen = true;
    s.trapFocusVisible || (s.trapFocusVisible = focusVisible);
}
