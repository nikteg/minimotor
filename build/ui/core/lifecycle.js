...
import { allUiApps, clearUiApp, markUiAppWired, resetUiApps, uiApp, uiAppWired, uiSlot, withUiApp, } from "./state.js";
import { setTheme } from "./theme.js";
const overlay = uiSlot(() => ({
    seen: false,
    active: false,
    inPass: false,
    held: false,
...
export function isInOverlayPass() {
...
export function captureOverlay(focusVisible = false) {
    const o = overlay();
    o.seen = true;
...
export function onOverlayPass(fn) {
    if (!overlayPassHooks.includes(fn))
        overlayPassHooks.push(fn);
}
...
export function onFrameEnd(fn) {
    if (!frameEndHooks.includes(fn))
        frameEndHooks.push(fn);
}
...
export function lifecycleOnce(register) {
    let done = false;
    return () => {
        if (done)
            return;
...
function appFrameEnd(app) {
    withUiApp(app, () => {
        ...
        for (const hook of overlayPassHooks)
            hook();
