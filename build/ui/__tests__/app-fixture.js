import { registerUiApp } from "../../ui/core/state.js";
const noop = () => { };
const unsubscribe = () => { };
...
const stepHandlers = new WeakMap();
...
export function endTestFrame(app) {
    for (const h of frameHandlers.get(app) ?? [])
        h();
}
...
export function stepTestApp(app) {
    for (const h of stepHandlers.get(app) ?? [])
        h();
}
/** An explicit app boundary for low-level widget tests. */
export function createTestUiApp(ctx) {
    const handlers = [];
    const steps = [];
