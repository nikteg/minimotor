...
export function navigation(pad, options = {}) {
    ...
    navigationState.x = Math.max(-1, Math.min(1, sx + Number(pad.down(Buttons.DpadRight)) - Number(pad.down(Buttons.DpadLeft))));
    navigationState.y = Math.max(-1, Math.min(1, sy + Number(pad.down(Buttons.DpadDown)) - Number(pad.down(Buttons.DpadUp))));
...
export function createGamepadTracker(read) {
    let connected = false;
    const held = [];
    const pressed = [];
    const released = [];
